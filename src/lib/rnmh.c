 ...
 */
    ...
    if (get_Nl_value(X_NCSIZE, (struct drive_Nl *)NULL, &v) < 0 || !v ||
        ...
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: WARNING: can't read name cache hash size: %s\n", ProgramName,
                          print_kptr(v, (char *)NULL, 0));
        return;
    }
    if (khsz < 1) {
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: WARNING: name cache hash size length error: %#lx\n",
                          ProgramName, khsz);
        return;
    }
    /*
 ...
 */
    ...
    if (get_Nl_value(X_NCACHE, (struct drive_Nl *)NULL, &v) < 0 || !v ||
        ...
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: WARNING: unusable name cache hash pointer: (%s)=%s\n",
                          ProgramName, print_kptr(v, tbuf, sizeof(tbuf)),
 ...
 */
    ...
    if (kread((KA_T)ka, (char *)khp, len)) {
        (void)fprintf(stderr, "%s: can't read name cache hash pointers from: %s\n", ProgramName,
                      print_kptr(ka, (char *)NULL, 0));
        return;
    }
    /*
 ...
 */
     ...
     */
        for (ka = (KA_T)khp[kx], n = 0; ka; ka = knx, n++) {
            if (n > lim) {
                if (!OptWarnings)
                    (void)fprintf(stderr, "%s: WARNING: name cache hash chain too long\n",
                                  ProgramName);
                break;
            }
        ...
        */
         ...
         */
...

            {
                ...
                if (!lc) {
                    (void)fprintf(stderr, "%s: can't allocate %d local name cache bytes\n",
                                  ProgramName, nlcl);
                    Exit(1);
                }
                lcl = nlcl;
 ...
 */
    ...
    if (!(Nchash = (struct l_nch **)calloc(len, sizeof(struct l_nch *)))) {
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: no space for %d local name cache hash pointers\n",
                          ProgramName, len);
        Exit(1);
    }
