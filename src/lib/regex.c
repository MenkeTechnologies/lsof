...
#define REGEX_REALLOCATE(source, osize, nsize) \
    (destination = (char *)alloca(nsize), memcpy(destination, source, osize))
...
static int re_match_2_internal PARAMS((struct re_pattern_buffer * bufp, const char *string1,
...
static void extract_number(dest, source) int *dest;
...
#ifdef DEBUG
static void extract_number_and_incr _RE_ARGS((int *destination, unsigned char **source));
static void extract_number_and_incr(destination, source) int *destination;
unsigned char **source;
{
    extract_number(destination, *source);
    *source += 2;
...
#define EXTRACT_NUMBER_AND_INCR(dest, src) extract_number_and_incr(&dest, &src)
#endif /* not EXTRACT_MACROS */
...
void print_fastmap(fastmap) char *fastmap;
{
...
void print_partial_compiled_pattern(start, end) unsigned char *start;
unsigned char *end;
{
    int mcnt, mcnt2;
    unsigned char *p1;
...
#else
            printf("/on_failure_keep_string_jump to %ld", (long int)(p + mcnt - start));
#endif
            break;
...
#else
            printf("/succeed_n to %ld, %d times", (long int)(p1 - start), mcnt2);
#endif
            break;
...
#else
            printf("/set_number_at location %ld to %d", (long int)(p1 - start), mcnt2);
#endif
            break;
...
void print_compiled_pattern(bufp) struct re_pattern_buffer *bufp;
{
...
void print_double_string(where, string1, size1, string2, size2) const char *where;
const char *string1;
const char *string2;
int size1;
int size2;
...
static const size_t re_error_msgid_idx[] = {
    REG_NOERROR_IDX, REG_NOMATCH_IDX, REG_BADPAT_IDX, REG_ECOLLATE_IDX, REG_ECTYPE_IDX,
    REG_EESCAPE_IDX, REG_ESUBREG_IDX, REG_EBRACK_IDX, REG_EPAREN_IDX,   REG_EBRACE_IDX,
    REG_BADBR_IDX,   REG_ERANGE_IDX,  REG_ESPACE_IDX, REG_BADRPT_IDX,   REG_EEND_IDX,
    REG_ESIZE_IDX,   REG_ERPAREN_IDX};
...
#define INIT_FAIL_STACK()                                                                      \
    do {                                                                                       \
        fail_stack.stack = (fail_stack_elt_t *)REGEX_ALLOCATE_STACK(INIT_FAILURE_ALLOC *       \
...
#define DOUBLE_FAIL_STACK(fail_stack)                                             \
    ((fail_stack).size > (unsigned)(re_max_failures * MAX_FAILURE_ITEMS)          \
         ? 0                                                                      \
         : ((fail_stack).stack = (fail_stack_elt_t *)REGEX_REALLOCATE_STACK(      \
                (fail_stack).stack, (fail_stack).size * sizeof(fail_stack_elt_t), \
                ((fail_stack).size << 1) * sizeof(fail_stack_elt_t)),             \
                                                                                  \
            (fail_stack).stack == NULL ? 0 : ((fail_stack).size <<= 1, 1)))

/* Push pointer POINTER on FAIL_STACK.
   Return 1 if was able to do so and 0 if ran out of memory allocating
   space to do so.  */
#define PUSH_PATTERN_OP(POINTER, FAIL_STACK)               \
    ((FAIL_STACK_FULL() && !DOUBLE_FAIL_STACK(FAIL_STACK)) \
         ? 0                                               \
         : ((FAIL_STACK).stack[(FAIL_STACK).avail++].pointer = POINTER, 1))
       ...
       be assigned */                 \
        ...
        if (1)                                                                                \
            for (this_reg = lowest_active_reg; this_reg <= highest_active_reg; this_reg++) {  \
                ...
                DEBUG_PRINT2(" match_null=%d", REG_MATCH_NULL_STRING_P(reg_info[this_reg]));  \
                DEBUG_PRINT2(" active=%d", IS_ACTIVE(reg_info[this_reg]));                    \
                DEBUG_PRINT2(" matched_something=%d", MATCHED_SOMETHING(reg_info[this_reg])); \
                DEBUG_PRINT2(" ever_matched=%d", EVER_MATCHED_SOMETHING(reg_info[this_reg])); \
                DEBUG_PRINT1("\n");                                                           \
...
#define NUM_FAILURE_ITEMS \
    (((0 ? 0 : highest_active_reg - lowest_active_reg + 1) * NUM_REG_ITEMS) + NUM_NONREG_ITEMS)
    ...
    do {                                                                                  \
        if (!set_regs_matched_done) {                                                     \
            ...
            for (r = lowest_active_reg; r <= highest_active_reg; r++) {                   \
                MATCHED_SOMETHING(reg_info[r]) = EVER_MATCHED_SOMETHING(reg_info[r]) = 1; \
            }                                                                             \
        }                                                                                 \
    } while (0)
...
static reg_errcode_t regex_compile _RE_ARGS((const char *pattern, size_t size, reg_syntax_t syntax,
...
static void insert_op2 _RE_ARGS((re_opcode_t op, unsigned char *loc, int arg1, int arg2,
                                 unsigned char *end));
...
static reg_errcode_t compile_range _RE_ARGS((unsigned int range_start, const char **p_ptr,
...
#define MOVE_BUFFER_POINTER(P) (__ptrlow(P) += incr, SET_HIGH_BOUND(P), __ptrvalue(P) += incr)
...
#define IS_CHAR_CLASS(string)                                                       \
    (STREQ(string, "alpha") || STREQ(string, "upper") || STREQ(string, "lower") ||  \
     STREQ(string, "digit") || STREQ(string, "alnum") || STREQ(string, "xdigit") || \
     STREQ(string, "space") || STREQ(string, "print") || STREQ(string, "punct") ||  \
     STREQ(string, "graph") || STREQ(string, "cntrl") || STREQ(string, "blank"))
#endif
...
static regex_grow_registers(num_regs) int num_regs;
{
...
static boolean group_in_compile_stack _RE_ARGS((compile_stack_type compile_stack, regnum_t regnum));
...
struct re_pattern_buffer *bufp;
{
    ...
    while (p != pend) {
               ...
               and also whether or not two or more matches is allowed.  */
                if (many_times_ok) { /* More than one repetition is allowed, so put in at the
                   ...
                   for null bytes, because of RE_DOT_NOT_NULL?  */
                    if (TRANSLATE(*(p - 2)) == TRANSLATE('.') && zero_times_ok && p < pend &&
                ...
                GET_BUFFER_SPACE(3);
                INSERT_JUMP(keep_string_p ? on_failure_keep_string_jump : on_failure_jump,
                            laststart, b + 3);
                pending_exact = 0;
                b += 3;
        ...
        case '[': {
            ...
            if ((re_opcode_t)b[-2] == charset_not && (syntax & RE_HAT_LISTS_NOT_NEWLINE))
                SET_LIST_BIT('\n');
            ...
            for (;;) {
                ...
                if (c == '-' && !(p - 2 >= pattern && p[-2] == '[') &&
                    !(p - 3 >= pattern && p[-3] == '[' && p[-2] == '^') && *p != ']') {
                    reg_errcode_t ret = compile_range(range_start, &p, pend, translate, syntax, b);
                    if (ret != REG_NOERROR)
                    ...
                    range_start = 0xffffffff;
                         ...
                         p[1] != ']') { /* This handles ranges made up of characters only.  */
                    reg_errcode_t ret;
                ...
                /* See if we're at the beginning of a possible character
                   ...
                   avoid an arbitrary limit in some compiler.  */
                            if ((is_alnum && ISALNUM(ch)) || (is_alpha && ISALPHA(ch)) ||
                                (is_blank && ISBLANK(ch)) || (is_cntrl && ISCNTRL(ch)))
                                SET_LIST_BIT(ch);
                            if ((is_digit && ISDIGIT(ch)) || (is_graph && ISGRAPH(ch)) ||
                                (is_lower && ISLOWER(ch)) || (is_print && ISPRINT(ch)))
                                SET_LIST_BIT(ch);
                            if ((is_punct && ISPUNCT(ch)) || (is_space && ISSPACE(ch)) ||
                                (is_upper && ISUPPER(ch)) || (is_xdigit && ISXDIGIT(ch)))
                                SET_LIST_BIT(ch);
                            if (translate && (is_upper || is_lower) && (ISUPPER(ch) || ISLOWER(ch)))
                                SET_LIST_BIT(ch);
                        }
...
#include <locale/weight.h>
                            ...
                            extra =
                                (const unsigned char *)_NL_CURRENT(LC_COLLATE, _NL_COLLATE_EXTRAMB);
                            indirect =
                                (const int32_t *)_NL_CURRENT(LC_COLLATE, _NL_COLLATE_INDIRECTMB);
                 ...
                 would allow matching multibyte sequences.  */
                     ...
                     the weight.  */
                                        ...
                                        while (cnt < len &&
                                               (weights[idx + 1 + cnt] == weights[idx2 + 1 + cnt]))
                                            ++len;
               ...
               representation.  */
                   ...
                   actually from exactly one character.  */
                            ...
                            symb_table =
                                (const int32_t *)_NL_CURRENT(LC_COLLATE, _NL_COLLATE_SYMB_TABLEMB);
         ...
         clear pending_exact explicitly.  */
            ...
            handle_close:
                ...
                assert(compile_stack.avail != 0);
                {
                   ...
                   as in `(ab)c(de)' -- the second group is #2.  */
                    ...
                    fixup_alt_jump = COMPILE_STACK_TOP.fixup_alt_jump
           ...
           clear pending_exact explicitly.  */
            ...
            handle_interval: {
                ...
                if (lower_bound < 0 || upper_bound > RE_DUP_MAX || lower_bound > upper_bound) {
                    if (!(syntax & RE_INTERVALS) && (syntax & RE_NO_BK_BRACES))
                        goto unfetch_interval;
                    else
                        FREE_STACK_RETURN(REG_BADBR);
                ...
                /* Otherwise, we have a nontrivial interval.  When
                    ...
                    INSERT_JUMP2(succeed_n, laststart, b + 5 + (upper_bound > 1) * 5, lower_bound);
                    b += 5;
        ...
        normal_char:
            ...
            if (!pending_exact
                ...
                || *p == '*' || *p == '^' ||
                ((syntax & RE_BK_PLUS_QM) ? *p == '\\' && (p[1] == '+' || p[1] == '?')
                                          : (*p == '+' || *p == '?')) ||
                ((syntax & RE_INTERVALS) &&
                 ((syntax & RE_NO_BK_BRACES) ? *p == '{' : (p[0] == '\\' && p[1] == '{')))) {
       ...
       is 2 * re_max_failures failure points.  */
        if (fail_stack.size < (2 * re_max_failures * MAX_FAILURE_ITEMS)) {
            ...
            if (!fail_stack.stack)
                ...
                fail_stack.stack = (fail_stack_elt_t *)xrealloc(
                    fail_stack.stack, (fail_stack.size * sizeof(fail_stack_elt_t)));
#else  /* not emacs */
            if (!fail_stack.stack)
                fail_stack.stack =
                ...
                fail_stack.stack = (fail_stack_elt_t *)realloc(
                    fail_stack.stack, (fail_stack.size * sizeof(fail_stack_elt_t)));
#endif /* not emacs */
        }
...
reg_syntax_t syntax;
{
    ...
    return
        ...
        (syntax & RE_NO_BK_PARENS ? *next == ')' : next_backslash && next_next && *next_next == ')')
...
static reg_errcode_t compile_range(range_start_char, p_ptr, pend, translate, syntax, b)
...
#if _LIBC
    collseq = (const unsigned char *)_NL_CURRENT(LC_COLLATE, _NL_COLLATE_COLLSEQMB);
...
struct re_pattern_buffer *bufp;
{
    ...
    while (1) {
        ...
        case dummy_failure_jump:
            ...
            if (!FAIL_STACK_EMPTY() && fail_stack.stack[fail_stack.avail - 1].pointer == p)
                fail_stack.avail--;
...
weak_alias(__re_search, re_search)
   ...
   stack overflow).  */
    ...
    int re_search_2(bufp, string1, size1, string2, size2, startpos, range, regs, stop)
struct re_pattern_buffer *bufp;
...
int stop;
{
    ...
    if (bufp->used > 0 && range > 0 &&
        ((re_opcode_t)bufp->buffer[0] == begbuf
         ...
         || ((re_opcode_t)bufp->buffer[0] == begline && !bufp->newline_anchor))) {
        if (startpos > 0)
            return -1;
        else
            range = 1;
    ...
    for (;;) {
        ...
        if (fastmap && startpos < total_size && !bufp->can_be_null) {
            ...
            {
                register char c = (size1 == 0 || startpos >= size1 ? string2[startpos - size1]
                                                                   : string1[startpos]);
...
#define WORDCHAR_P(d) \
    (SYNTAX((d) == end1 ? *string2 : (d) == string2 - 1 ? *(end1 - 1) : *(d)) == Sword)
...
static boolean alt_match_null_string_p _RE_ARGS((unsigned char *p, unsigned char *end,
                                                 register_info_type *reg_info));
static boolean common_op_match_null_string_p _RE_ARGS((unsigned char **p, unsigned char *end,
                                                       register_info_type *reg_info));
...
weak_alias(__re_match_2, re_match_2)
   ...
   afterwards.  */
    static int re_match_2_internal(bufp, string1, size1, string2, size2, pos, regs, stop)
struct re_pattern_buffer *bufp;
const char *string1, *string2;
int size1, size2;
int pos;
...
int stop;
{
    ...
    if (bufp->re_nsub) {
        ...
        if (!(regstart && regend && old_regstart && old_regend && reg_info && best_regstart &&
        ...
        regstart = regend = old_regstart = old_regend = best_regstart = best_regend = reg_dummy =
            NULL;
    ...
    for (;;) {
        ...
        if (p == pend) { /* End of pattern means we might have succeeded.  */
         ...
         as the best previous match.  */
                boolean same_str_p = (FIRST_STRING_P(match_end) == MATCHING_IN_FIRST_STRING);
        ...
        succeed_label:
            ...
            if (regs && !bufp->no_sub) {
                ...
                if (bufp->regs_allocated ==
                    REGS_UNALLOCATED) { /* No.  So allocate them with malloc.  We need one
                    ...
                    bufp->regs_allocated = REGS_REALLOCATE;
                } else if (bufp->regs_allocated ==
                           REGS_REALLOCATE) { /* Yes.  If we need more elements than were already
        ...
        case stop_memory:
            ...
            /* If just failed to match something this time around with a
             ...
             last match.  */
            if ((!MATCHED_SOMETHING(reg_info[*p]) || just_past_start_mem == p - 1) &&
                (p + 2) < pend) {
        ...
        case duplicate: {
            ...
            dend2 =
                ((FIRST_STRING_P(regstart[regno]) == FIRST_STRING_P(regend[regno])) ? regend[regno]
                                                                                    : end_match_1);
            for (;;) {
       ...
       We change it to either `pop_failure_jump' or `jump'.  */
        case maybe_pop_jump:
           ...
           match at least one of that.  */
                while (1) {
                    if (p2 + 2 < pend &&
                        ((re_opcode_t)*p2 == stop_memory || (re_opcode_t)*p2 == start_memory))
                        p2 += 3;
           ...
           yet.  */
                   ...
                   of the charset.  */
                    if ((re_opcode_t)p1[3] == exactn &&
                        !((int)p2[1] * BYTEWIDTH > (int)p1[5] &&
                          (p2[2 + p1[5] / BYTEWIDTH] & (1 << (p1[5] % BYTEWIDTH))))) {
                        p[-3] = (unsigned char)pop_failure_jump;
                        DEBUG_PRINT1("  No match => pop_failure_jump.\n");
                    }
               ...
               lists every character listed in the charset after.  */
                        for (idx = 0; idx < (int)p2[1]; idx++)
                            if (!(p2[2 + idx] == 0 ||
                                  (idx < (int)p1[4] && ((p2[2 + idx] & ~p1[5 + idx]) == 0))))
                                break;
        ...
        case pop_failure_jump: {
            ...
            POP_FAILURE_POINT(sdummy, pdummy, dummy_low_reg, dummy_high_reg, reg_dummy, reg_dummy,
                              reg_info_dummy);
        }
            /* Note fall through.  */
    ...
    case notwordbound:
      ...
      break;
        ...
        case wordend:
            ...
            if (!AT_STRINGS_BEG(d) && WORDCHAR_P(d - 1) && (!WORDCHAR_P(d) || AT_STRINGS_END(d)))
                break;
            goto fail;
    ...
    fail:
        if (!FAIL_STACK_EMPTY()) { /* A restart point is known.  Restore to that state.  */
            DEBUG_PRINT1("\nFAIL:\n");
            POP_FAILURE_POINT(d, p, lowest_active_reg, highest_active_reg, regstart, regend,
                              reg_info);
        ...
        re_comp(s) const char *s;
{
    ...
    if (!re_comp_buf.buffer) {
        ...
        if (re_comp_buf.buffer == NULL)
            return (char *)gettext(re_error_msgid + re_error_msgid_idx[(int)REG_ESPACE]);
...
int
    ...
    weak_function
#endif
        re_exec(s) const char *s;
{
    const int len = strlen(s);
    return 0 <= re_search(&re_comp_buf, s, len, 0, len, (struct re_registers *)0);
}
...
int cflags;
{
    ...
    if (cflags & REG_ICASE) {
        ...
        preg->translate = (RE_TRANSLATE_TYPE)malloc(CHAR_SET_SIZE * sizeof(*(RE_TRANSLATE_TYPE)0));
