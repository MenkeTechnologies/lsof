...
static void report_WARNDEVACCESS(char *pfx, char *verb,
        char *punct);
...
static void
report_HASDCACHE(int type, char *ttl, char *det)
{
...
static void
report_HASKERNIDCK(char *pfx, char *verb)
{
...
static void
report_SECURITY(char *pfx, char *punct)
{
...
static void
report_WARNDEVACCESS(char *pfx, char *verb, char *punct)
{
    fprintf(stderr, "%s/dev warnings %s%s%s%s",
                   pfx ? pfx : "",
                   verb ? verb : "",
                   verb ? " " : "",
...
void
usage(int exit_val, int field_help, int version)
{
