...
char isglocked(struct gnode *ga) {
 ...
 */
    ...
    if (ISVDEV(g.gn_type)) {
        ...
        if (!g.gn_data || kread((KA_T)g.gn_data, (char *)&sn, sizeof(sn))) {
            ...
            (void)snpf(NameChars, NameCharsLength, "vnode at %s: can't read specnode (%s)",
        ...
        if (sn.sn_pfsgnode) {
            ...
            if (readgnode((KA_T)sn.sn_pfsgnode, &g)) {
                (void)snpf(NameChars, NameCharsLength, "vnode at %s: can't read pfsgnode (%s)",
                           print_kptr(va, tbuf, sizeof(tbuf)),
            ...
            if (!g.gn_data || readlino(&g, &i)) {
                (void)snpf(NameChars, NameCharsLength, "pfsgnode at %s: can't read inode (%s)",
            ...
            if (!g.gn_vnode || readvnode((KA_T)g.gn_vnode, v)) {
                (void)snpf(NameChars, NameCharsLength, "pfsgnode at %s: can't read vnode (%s)",
                           print_kptr((KA_T)sn.sn_pfsgnode, tbuf, sizeof(tbuf)),
            ...
            if (CloneMaj >= 0 && sn.sn_devnode &&
                 ...
                 */
                        ...
                        if (ClonePtc >= 0 && GET_MAJ_DEV(g.gn_rdev) == ClonePtc) {
                            if (SelectInetOnly) {
                                CurrentLocalFile->sel_flags = SELEXCLF;
                                return;
                            }
                     ...
                     */
                            (void)snpf(NameChars, NameCharsLength, "%s/%d", cl->cd.name,
                 ...
                 */
                     ...
                     */
                            ...
                            if (!(ka = (KA_T)qi.qi_minfo) || kread(ka, (char *)&mi, sizeof(mi)))
                                continue;
                            if (!(ka = (KA_T)mi.mi_idname) || kread(ka, mn, sizeof(mn) - 1) ||
                                !(ml = (int)strlen(mn)) || !strcmp(mn, "sth"))
                                continue;
                            if (!strcmp(mn, "xtiso") && (xp = (KA_T)q.q_ptr) &&
                                !kread(xp, (char *)&xt, sizeof(xt)) && (ka = (KA_T)xt.xti_so)) {
         ...
         */
            ...
            if (!sn.sn_devnode || kread((KA_T)sn.sn_devnode, (char *)&dn, sizeof(dn))) {
                (void)snpf(NameChars, NameCharsLength, "vnode at %s: can't read devnode (%s)",
                           print_kptr(va, tbuf, sizeof(tbuf)),
    ...
    case N_NFS:
        ...
        if (width == -1) {
            ...
            if (__KERNEL_64()) {
                ...
                if (!OptWarnings)
                    (void)fprintf(stderr, "%s: WARNING: unknown kernel bit size\n", ProgramName);
                width = -2;
            }
#endif /* AIXV<-4330 */
        }
        ...
        if (width > 0) {
            if (!g.gn_data || kread((KA_T)g.gn_data, rp, rsz)) {
                (void)snpf(NameChars, NameCharsLength, "remote gnode at %s has no rnode",
 ...
 */
    case N_REGLR:
        if (vfs && vfs->vmt_gfstype == MNT_CDROM) {
            ...
            if (readlino(&g, &i)) {
                (void)snpf(NameChars, NameCharsLength, "gnode at %s can't read inode: %s",
 ...
 */
    if (OptOffset)
        ...
        case N_REGLR:
            ...
            } else if (((type == VBLK) || (type == VCHR) || (type == VMPC)) && !OptSize)
                CurrentLocalFile->off_def = 1;
            break;
        }
    }
...
int readlino(struct gnode *ga, struct l_ino *li) {
