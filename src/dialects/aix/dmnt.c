    ...
    for (v = vt; nm--; v = (struct vmount *)((char *)v + v->vmt_length)) {
        ...
        if (statsafely(dir, &sb)) {
            if (!OptWarnings) {
                ...
                (void)fprintf(stderr, "%s: WARNING: can't stat() %s file system %s\n", ProgramName,
                              ty, dir);
            ...
            if (v->vmt_flags & MNT_REMOTE) {
...
#else  /* AIXA>=2 */
                sb.st_dev = (dev_t)(SDEV_REMOTE | (SDEV_REMOTE << 32) | v->vmt_vfsnumber);
#endif /* AIXA<2 */
                 ...
                 */
                    sb.st_dev = (dev_t)(v->vmt_fsid.val[0] & 0x7fffffffffffffff);
                    /*
                 * !!!DEBUG!!!   !!!DEBUG!!!   !!!DEBUG!!!   !!!DEBUG!!!
                  */
            ...
            if (!OptWarnings)
                (void)fprintf(stderr, "      assuming \"dev=%#lx\" from mount table\n", sb.st_dev);
