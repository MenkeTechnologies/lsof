...
#define SOHASH(d, n) \
    ((((int)(((GET_MIN_DEV(d) & 0x7fffffff) * SOHASHBUCKS) + n) * 31415) >> 7) & (SOHASHBUCKS - 1))
 ...
 */
    ...
    while (((np = getproc(P, Np, PROCSIZE)) == -1) && errno == ENOSPC) {
        Np = P->p_pid + 10;
        if (!(P = (struct PROCINFO *)realloc((MALLOC_P *)P, (size_t)(Np * PROCSIZE)))) {
    ...
    np = pid = 0;
    p = P;
    while ((i = GETPROCS(p, PROCSIZE, (struct FDSINFO *)NULL, 0, &pid, PROCINFO_INCR)) ==
           PROCINFO_INCR) {
        np += PROCINFO_INCR;
        if (np >= Np) {
 ...
 */
    for (p = P, Up = &us; np > 0; np--, p++) {
        if (p->p_stat == 0 || p->p_stat == SZOMB)
            continue;
        if (is_proc_excl(p->p_pid, (int)p->p_pgid, (UID_ARG)p->p_uid, &pss, &sf))
            continue;
...
#if AIXV >= 4110
        if (OptCrossoverExt && kreadx(Uo, (char *)Up, U_SIZE, (KA_T)p->pi_adspace) == 0)
            i = 1;
        else
            i = 0;
        if (i == 0) {
        ...
        if (!fds) {
            if (!(fds = (struct FDSINFO *)malloc((MALLOC_S)FDSINFOSIZE))) {
                (void)fprintf(stderr, "%s: can't allocate fdsinfo struct for PID %d\n", ProgramName,
                              pid);
                Exit(1);
            }
        }
        pid = p->p_pid;
        if (GETPROCS((struct PROCINFO *)NULL, PROCSIZE, fds, FDSINFOSIZE, &pid, 1) != 1)
            continue;
        hl = 0;
        ...
        if (OptCrossoverExt) {
             ...
             */
                if (kreadx((KA_T)((char *)Uo + offsetof(struct user, U_loader) + uo),
                           (char *)&Up->U_loader, sizeof(struct la), (KA_T)p->pi_adspace))
                    break;
                if (trx) {
                    hl = 1;
                    break;
             ...
             */
                if (!kreadx((KA_T)((char *)Uo + offsetof(struct user, U_maxofile) + uo),
                ...
                if (uo == 0)
                    ...
                    trx = 1;
                    if (!OptWarnings) {
                        (void)fprintf(stderr, "%s: WARNING: user struct mismatch;", ProgramName);
...
#else  /* AIXV!=4330 */
        if (OptCrossoverExt &&
            kreadx((KA_T)((char *)Uo + offsetof(struct user, U_loader)), (char *)&Up->U_loader,
                   sizeof(struct la), (KA_T)p->pi_adspace) == 0)
            hl = 1;
#endif /* AIXV==4330 */
         ...
         */
        alloc_lproc(p->p_pid, (int)p->p_pgid, (int)p->p_ppid, (UID_ARG)p->p_uid, cmd, (int)pss,
                    (int)sf);
        ...
        if (!ckscko && hl) {
...
#else  /* AIXA==1 */
            {
                ...
                if ((Up->U_loader[0] & URDXMASK) || (Up->U_loader[1] & URDXMASK)) {
 ...
 */
    ...
    if ((AIX_KERNBITS == 0) || !kb || (kb != AIX_KERNBITS)) {
        if (AIX_KERNBITS == 32)
            ...
            kbb = "unknown";
        (void)fprintf(stderr, "%s: FATAL: compiled for a kernel of %s bit size.\n", ProgramName,
                      kbb);
 ...
 */
    ...
    if (OptCrossoverExt) {
...
#if AIXA < 2
        struct nlist ll[] = {{"library_anchor"},
        ...
        if (nlist(N_UNIX, ll) == 0 && ll[0].n_value != (long)0 && ll[1].n_value != (long)0 &&
            kreadx((KA_T)(ll[1].n_value & RDXMASK), (char *)&Soff, sizeof(Soff), (KA_T)0) == 0)
            Soff_stat++;
#endif /* AIXA<2 */
...
static void getlenm(struct le *le, KA_T sid) {
    ...
    if (!sid) {
        ...
        if (!Soff_stat || !le->nm || kreadx((KA_T)le->nm & (KA_T)RDXMASK, buf, LIBNMLN, (KA_T)Soff))
            return;
    }
    buf[LIBNMLN - 1] = '\0';
    i = strlen(buf);
 ...
 */
    ...
    if ((MyRealUid != 0) && !SetuidRootState && !OptWarnings) {
        (void)fprintf(stderr, "%s: WARNING: insufficient permission to access all", ProgramName);
        (void)fprintf(stderr, " /%s/object sub-\n", HASPROCFS);
        (void)fprintf(stderr, "      directories; some loader information may", ProgramName);
 ...
 */
    if ((ct = ldr_get_modules(SOL_GLOBAL, (void *)buf, &bufsz)) < 1)
        return;
    if (!(SoHash = (so_hash_t **)calloc((MALLOC_S)SOHASHBUCKS, sizeof(so_hash_t *)))) {
...
int kreadx(KA_T addr, char *buf, int len, KA_T sid) {
    int br;
 ...
 */
     ...
     */
        (void)snpf(fd, sizeof(fd), "L%02d", i++);
        alloc_lfile(fd, -1);
        CurrentLocalFile->dev_def = CurrentLocalFile->inp_ty = CurrentLocalFile->nlink_def =
     ...
     */
        for (sp = SoHash[SOHASH(sb.st_dev, (INODETYPE)sb.st_ino)]; sp; sp = sp->next) {
