...
static void get_lock_state(KA_T f) {
...
void process_kqueue(ka) KA_T ka; /* kqueue file structure address */
{
 ...
 */
    ...
    /*
                 ...
                 */
    if (((v->v_type == VCHR) || (v->v_type == VBLK)) && v->v_rdev
 ...
 */
...
#if !defined(HAS_ZFS)
            if (!OptWarnings && !zw) {
                (void)fprintf(stderr, "%s: WARNING: no ZFS support has been defined.\n",
                              ProgramName);
                (void)fprintf(stderr, "      See 00FAQ for more information.\n");
                zw = 1;
            }
#else  /* defined(HAS_ZFS) */
            vtag = VT_ZFS;
...
#if FREEBSDV >= 5000
    case VT_DEVFS:
        if (!v->v_data || kread((KA_T)v->v_data, (char *)&de, sizeof(de))) {
        ...
        d = &de;
        if (v->v_type == VDIR) {
            if (!d->de_dir || kread((KA_T)d->de_dir, (char *)&de, sizeof(de))) {
...
#if defined(HAS9660FS)
    case VT_ISOFS:
        if (read_iso_node(v, &iso_dev, &iso_dev_def, &iso_ino, &iso_links, &iso_sz)) {
...
#else  /* FREEBSDV>=2000 */
        if (!v->v_data || kread((KA_T)v->v_data, (char *)&mb, sizeof(mb))) {
...
#else  /* FREEBSDV>=2000 */
        if (!v->v_data || kread((KA_T)v->v_data, (char *)&nb, sizeof(nb))) {
        ...
        if (sc == 1) {
            ...
            if (vfs && (vfs->dir || vfs->fsname || vfs->fsid.val[0])) {
                if (vfs->fsid.val[0]) {
                    ...
                    (void)snpf(dbuf, sizeof(dbuf) - 1, "%d,%d", GET_MAJ_DEV(dev), GET_MIN_DEV(dev));
                ...
                (void)snpf(tbuf, sizeof(tbuf) - 1, "(nullfs%s%s%s%s%s%s%s)",
                           (vfs && vfs->fsname) ? " " : "", (vfs && vfs->fsname) ? vfs->fsname : "",
                           (vfs && vfs->dir) ? " on " : "", (vfs && vfs->dir) ? vfs->dir : "",
                           (dp && vfs && vfs->dir) ? " (" : "", (dp && vfs && vfs->dir) ? dp : "",
                           (dp && vfs && vfs->dir) ? ")" : "");
...
#else  /* FREEBSDV>=2000 */
        if (!v->v_data || kread((KA_T)v->v_data, (char *)&pb, sizeof(pb))) {
...
#if defined(HASPSEUDOFS)
    case VT_PSEUDOFS:
        if (!v->v_data || kread((KA_T)v->v_data, (char *)&pn, sizeof(pn))) {
    ...
    case VT_UFS:
...
#else /* FREEBSDV>=2000 */
        if (!v->v_data || kread((KA_T)v->v_data, (char *)&ib, sizeof(ib))) {
        ...
        if (i->i_ump && !kread((KA_T)i->i_ump, (char *)&um, sizeof(um))) {
            if (um.um_fstype == UFS1) {
                if (i->i_din1 && !kread((KA_T)i->i_din1, (char *)&d1, sizeof(d1)))
                    ufst = 1;
            } else {
                if (i->i_din2 && !kread((KA_T)i->i_din2, (char *)&d2, sizeof(d2)))
                    ufst = 2;
            }
        }
#endif /* defined(HAS_UFS1_2) */
    ...
    case VT_ZFS:
        if (!v->v_data ||
            (zm = readzfsnode((KA_T)v->v_data, &zi, ((v->v_vflag & VV_ROOT) ? 1 : 0)))) {
 ...
 */
    ...
    if (n) {
        ...
        if (f->fd_link && kread((KA_T)f->fd_link, NameChars, NameCharsLength - 1) == 0)
...
#if FREEBSDV < 2000
            if (type == VDIR || !p || !p->pfs_vs || kread((KA_T)p->pfs_vs, (char *)&vm, sizeof(vm)))
                break;
            if (pgsz < 0)
                pgsz = getpagesize();
            CurrentLocalFile->sz =
                (SZOFFTYPE)((pgsz * vm.vm_tsize) + (pgsz * vm.vm_dsize) + (pgsz * vm.vm_ssize));
            CurrentLocalFile->sz_def = 1;
            break;
#else  /* FREEBSDV>=2000 */
            if (p) {
 ...
 */
    ...
    if (type == VBAD)
        ...
        if (type == VDIR)
            ...
            (void)snpf(NameChars, NameCharsLength, "/%s/%0*d", HASPROCFS, PNSIZ, p->pfs_pid);
        enter_nm(NameChars);
#else /* FREEBSDV>=2000 */
        ty = (char *)NULL;
        (void)snpf(NameChars, NameCharsLength, "/%s", HASPROCFS);
...
void process_pipe(pa) KA_T pa; /* pipe structure address */
{
    char dev_ch[32], *ep;
    struct pipe p;
    size_t sz;
    ...
    (void)snpf(dev_ch, sizeof(dev_ch), "%s", print_kptr(pa, (char *)NULL, 0));
    enter_dev_ch(dev_ch);
    if (OptOffset)
        CurrentLocalFile->off_def = 1;
    else {
