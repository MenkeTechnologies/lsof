...
#if FREEBSDV < 2000
static char *mnt_names[] = {"none", "ufs", "nfs", "mfs", "pc", "iso9600", "procfs", "devfs"};
     ...
     */
         ...
         */
            if (((cp = strrchr(ln, ':')) && (*(cp + 1) == '/')) ||
                !strcasecmp(mb->f_fstypename, "cd9660") || !strcasecmp(mb->f_fstypename, "zfs")) {
                ss = 1;
                s = (u_int)0;
            }
        }
 ...
 */
...
#else  /* !defined(MOUNT_NONE) */
    {
        ...
        if ((len = strlen(m.m_stat.f_fstypename))) {
            if (len > (MFSNAMELEN - 1))
                len = MFSNAMELEN - 1;
            if (!(vp->typnm = mkstrcat(m.m_stat.f_fstypename, len, (char *)NULL, -1, (char *)NULL,
                                       -1, (MALLOC_S *)NULL))) {
                (void)fprintf(stderr, "%s: no space for fs type name: ", ProgramName);
