...
#define USESPOFILE 1
    ...
    if (!oftp) {
        ...
        if (oftsz != (ofasz * SFDCHUNK)) {
            (void)fprintf(stderr, "%s: FD chunk size (%d) not exact multiple of %d\n", ProgramName,
                          oftsz, SFDCHUNK);
            Exit(1);
        }
 ...
 */
...

    {
        ...
        if ((p->p_flag & SLOAD) == 0) {
            ...
            if (lseek(Swap, (off_t)dtob(sw), L_SET) == (off_t)-1 || read(Swap, u, U_SIZE) != U_SIZE)
                continue;
        } else {
            ...
            if (kread(pte_off, (char *)&pte1, sizeof(pte1)))
                continue;
            pte_addr = (KA_T)(ctob(pte1.pg_pfnum + 1) - ((UPAGES + FLOAT) * sizeof(pte2)));
            if (mread(pte_addr, (char *)&pte2, sizeof(pte2)))
                continue;
            if (mread((KA_T)ctob(pte2.pg_pfnum), (char *)u, sizeof(struct user)))
                continue;
#endif /* defined(hp9000s300) */

#if defined(hp9000s800)
            if (kread((KA_T)uvadd((struct proc *)Kpa), (char *)u, sizeof(struct user)))
                continue;
        }
#endif /* defined(hp9000s800) */
#endif /* HPUXV>=800 */
        ...
        if (is_cmd_excl(u->u_comm, &pss, &sf))
            continue;
        alloc_lproc(p->p_pid, (int)p->p_pgid, (int)p->p_ppid, (UID_ARG)p->p_uid, u->u_comm,
                    (int)pss, (int)sf);
...

        {
...
#if HPUXV >= 800
            if (j >= SFDCHUNK) {
                if (!pfp || kread((KA_T)pfp, (char *)&ofp, sizeof(ofp)) || !ofp ||
                    kread((KA_T)ofp, oftp, oftsz))
                    break;
                j = 0;
                pfp += sizeof(KA_T);
            ...
            if (j >= SFDCHUNK) {
                ...
                if (k >= NFDCHUNKS)
                    break;
                if (kread((KA_T)u->u_ofilep[k], (char *)&u->u_ofile, sizeof(struct ofile_t))) {
                    break;
                }
                j = 0;
            }
 ...
 */
         ...
         */
    {
        ...
        if (rv != (long)HPUXKERNBITS) {
            (void)fprintf(stderr, "%s: FATAL: %s was built for a %d bit kernel, but this\n",
 ...
 */
    ...
    if (get_Nl_value("proc", Drive_Nl, &v) < 0 || !v || kread((KA_T)v, (char *)&Kp, sizeof(Kp)) ||
        get_Nl_value("nproc", Drive_Nl, &v) < 0 || !v || kread((KA_T)v, (char *)&Np, sizeof(Np)) ||
        !Kp || Np < 1) {
...
static void process_text(KA_T vasp) {
     ...
     */
    for (i = lm = 0, prp = (KA_T)v.va_next; prp != vasp; prp = (KA_T)p.p_next, lm++) {
        ...
        if (i >= Nva) {
            ...
            if (!Vp) {
                (void)fprintf(stderr, "%s: no more space for text vnode pointers\n", ProgramName);
                Exit(1);
            }
        }
        Vp[i++] = va;
