...
void print_tcptpi(int nl) {
    ...
    if (OptTcpTpiInfo & TCPTPI_FLAGS) {
        ...
        if ((opt = CurrentLocalFile->lts.opt) || CurrentLocalFile->lts.qlens ||
            CurrentLocalFile->lts.qlims || CurrentLocalFile->lts.rbszs ||
            CurrentLocalFile->lts.sbsz) {
            char sep = ' ';
 ...
 */
    if (lcb.lla_flags & ~(LLA_FWRITE | LLA_FREAD)) {
        ep = endnm(&sz);
        (void)snpf(ep, sz, "%s(flags = %#x)", (ep == NameChars) ? "" : " ", lcb.lla_flags);
    }
    if (NameChars[0])
        enter_nm(NameChars);
}
 ...
 */
    ...
    case AF_CCITT:
        ...
        if (!xp.x25pcb_extend || kread((KA_T)xp.x25pcb_extend, (char *)&xpe, sizeof(xpe))) {
            (void)snpf(NameChars, NameCharsLength, "can't read x.25 pcb (%s) extension at %s",
        ...
        if (xpe.x25pcbx_peer_addr.x25hostlen > 0) {
            ...
            if (i * 2 != xpe.x25pcbx_peer_addr.x25hostlen) {
                ep = endnm(&sz);
                (void)snpf(ep, sz, "%01x", xpe.x25pcbx_peer_addr.x25_host[i] >> 4);
            }
        }
        enter_nm(NameChars);
        break;
 ...
 */
    case AF_INET:
        ...
        if (s.so_sth) {
             ...
             */
                if (!ip) {
                    (void)snpf(NameChars, NameCharsLength, "no IP module for stream socket");
                } else {
                    (void)snpf(NameChars, NameCharsLength, "no TCP/UDP module for stream socket");
                }
            }
            enter_nm(NameChars);
            return;
 ...
 */
    case AF_UNIX:
        ...
        mbl = 0;
        if (unp.unp_addr && kread((KA_T)unp.unp_addr, (char *)&mb, sizeof(mb)) == 0 && mb.b_datap &&
            ...
            if (db.db_base) {
                if (dbl < (db.db_size + 1)) {
                    ...
                    if (!dbf) {
                        (void)fprintf(stderr, "%s: no space (%d) for UNIX socket address\n",
                                      ProgramName, dbl);
                        Exit(1);
                    }
                }
...
void process_stream_sock(KA_T ip, KA_T pcb, char *pn, enum vtype vt) {
    unsigned char *fa = (unsigned char *)NULL;
    char *ep;
    int fp, lp, rq, sq;
    struct ipc_s ic;
 ...
 */
    ...
    if (pn) {
        ...
        CurrentLocalFile->inp_ty = 2;
    } else if (SearchFileChain && (vt != VNON) && CurrentLocalFile->dev_def &&
               (CurrentLocalFile->inp_ty == 1)) {
 ...
 */
    *NameChars = '\0';
    if (!ip || kread(ip, (char *)&ic, sizeof(ic))) {
        ep = endnm(&sz);
        (void)snpf(ep, sz, "%scan't read IP control structure from %s", sz ? " " : "",
                   print_kptr(ip, (char *)NULL, 0));
        enter_nm(NameChars);
        return;
    }
 ...
 */
    if (!pcb) {
        ep = endnm(&sz);
        (void)snpf(ep, sz, "%ssocket stream has no TCP or UDP module", sz ? " " : "");
        enter_nm(NameChars);
        return;
    }
    /*
 ...
 */
     ...
     */
        ...
        lp = (int)ntohs(pt);
        if ((int)ic.ipc_tcp_faddr != INADDR_ANY || (u_short)ic.ipc_tcp_fport != 0) {
     ...
     */
        ...
        (void)ent_inaddr(la, (int)ntohs(pt), (unsigned char *)NULL, -1, AF_INET);
        if (!OptSize)
            CurrentLocalFile->off_def = 1;
     ...
     */
        ep = endnm(&sz);
        (void)snpf(ep, sz, "%sunknown stream protocol: %s", sz ? " " : "", pn ? pn : "NUll");
    }
    if (NameChars[0])
        enter_nm(NameChars);
}
