 ...
 */
    ...
    /*
 * If any X_AFS_* symbol that doesn't have a value in NlistTable[] has one from
 * the AFS modload file, copy its modload value to NlistTable[].
 */
    if (((i = get_Nl_value("arFid", Drive_Nl, NULL)) >= 0) && !NlistTable[i].n_value &&
        nl[i].n_value)
        NlistTable[i].n_value = nl[i].n_value;
    if (((i = get_Nl_value("avops", Drive_Nl, NULL)) >= 0) && !NlistTable[i].n_value &&
        nl[i].n_value)
        NlistTable[i].n_value = nl[i].n_value;
    if (((i = get_Nl_value("avol", Drive_Nl, NULL)) >= 0) && !NlistTable[i].n_value &&
        nl[i].n_value)
        NlistTable[i].n_value = nl[i].n_value;
}
...
static struct volume *getvolume(struct VenusFid *f, int *vols) {
    ...
    if (!ka) {
        if (get_Nl_value("avol", Drive_Nl, (unsigned long *)&ka) < 0 || !ka) {
            if (!w && !OptWarnings) {
                (void)fprintf(stderr, "%s: WARNING: no afs_volumes kernel address\n", ProgramName);
 ...
 */
    if (AFSVfsp && !vp->v_data && vp->v_vfsp == AFSVfsp)
        return (1);
    if (vp->v_data || !vp->v_vfsp || kread((KA_T)vp->v_vfsp, (char *)&v, sizeof(v)) || v.vfs_data ||
...
int readafsnode(KA_T va, struct vnode *v, struct afsnode *an) {
    ...
    if (kread(ka, cp, len)) {
        (void)snpf(NameChars, NameCharsLength, "vnode at %s: can't read vcache remainder from %s",
                   print_kptr(va, tbuf, sizeof(tbuf)), print_kptr((KA_T)ka, (char *)NULL, 0));
        enter_nm(NameChars);
        return (1);
    }
    vc = (struct vcache *)v;
 ...
 */
    if (vc->mvstat == 2) {
        if ((vp = getvolume(&vc->fid, &vols))) {
            an->inode = (INODETYPE)(vp->mtpoint.Fid.Vnode + (vp->mtpoint.Fid.Volume << 16));
