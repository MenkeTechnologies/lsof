...
_PROTOTYPE(static void make_sock,
           (struct pst_fileinfo2 * f, struct pst_stream *sh, struct pst_socket *s));
...
static void make_sock(struct pst_fileinfo2 *f, struct pst_stream *sh, struct pst_socket *s) {
    size_t sz;
    /*
...
#if defined(PS_STR_XPORT_TS_UNBND) && defined(PS_TS_UNBND) && (PS_STR_XPORT_TS_UNBND != PS_TS_UNBND)
...
#if defined(PS_STR_XPORT_TS_IDLE) && defined(PS_TS_IDLE) && (PS_STR_XPORT_TS_IDLE != PS_TS_IDLE)
 ...
 */
    ...
    if ((s->pst_boundaddr_len = sz)) {
        (void)memcpy((void *)s->pst_boundaddr, (const void *)sh->pst_str_xport_boundaddr, sz);
    }
    sz = (size_t)sh->pst_str_xport_remaddr_len;
    if (sz > sizeof(s->pst_remaddr))
        sz = sizeof(s->pst_remaddr);
    if ((s->pst_remaddr_len = sz)) {
        (void)memcpy((void *)s->pst_remaddr, (const void *)sh->pst_str_xport_remaddr, sz);
    }
}
#endif /* defined(PS_STR_XPORT_DATA) */
...
void print_tcptpi(int nl) {
    ...
    if (OptTcpTpiInfo & TCPTPI_STATE) {
        ...
        case 0: /* TCP */
            ...
            if ((i = CurrentLocalFile->lts.state.val + TcpStateOffset) < 0 || i >= TcpNumStates) {
                (void)snpf(sbuf, sizeof(sbuf), "UknownState_%d", CurrentLocalFile->lts.state.val);
                cp = sbuf;
            } else
                cp = TcpStateNames[i];
            break;
        case 1: /* UDP */
            if (!UdpStateNames)
                (void)build_IPstates();
            if ((u = CurrentLocalFile->lts.state.uval + UdpStateOffset) > UdpNumStates) {
    ...
    if (OptTcpTpiInfo & TCPTPI_FLAGS) {
        ...
        if ((opt = CurrentLocalFile->lts.opt) || CurrentLocalFile->lts.qlens ||
            CurrentLocalFile->lts.qlims) {
            char sep = ' ';
 ...
 */
    if (!s) {
        if (!(s = read_sock(f))) {
            (void)snpf(NameChars, NameCharsLength, "can't read pst_socket%s%s", errno ? ": " : "",
 ...
 */
    ...
    case PS_AF_INET:
    case PS_AF_INET6:
        CurrentLocalFile->lts.recv_queue = (unsigned long)s->pst_idata;
        CurrentLocalFile->lts.send_queue = (unsigned long)s->pst_odata;
     ...
     */
    CurrentLocalFile->lts.sock_state = (unsigned int)s->pst_state;
#endif /* defined(HASSOSTATE) */
    ...
    case PS_AF_INET:
    case PS_AF_INET6:
        CurrentLocalFile->lts.read_win = (unsigned long)s->pst_rwnd;
        CurrentLocalFile->lts.write_win = (unsigned long)s->pst_swnd;
        CurrentLocalFile->lts.read_win_st = CurrentLocalFile->lts.write_win_st = (unsigned char)1;
    }
#endif /* defined(HASTCPTPIW) */
 ...
 */
    ...
    case PS_AF_INET:
        ...
        case PS_PROTO_TCP:
            ...
            CurrentLocalFile->lts.state.val = (int)s->pst_pstate;
            break;
        case PS_PROTO_UDP:
            CurrentLocalFile->lts.type = 1;
            CurrentLocalFile->lts.state.uval = (unsigned int)s->pst_pstate;
        }
        /*
    ...
    case PS_AF_INET6:
        ...
        if ((size_t)s->pst_remaddr_len == sizeof(struct sockaddr_in6)) {
            ...
            if ((fp = (int)htons(sa6->sin6_port)) || !IN6_IS_ADDR_UNSPECIFIED(&sa6->sin6_addr))
    ...
    case PS_AF_UNIX:
        ...
        if (((len = (size_t)s->pst_boundaddr_len) > 0) && (len <= sizeof(struct sockaddr_un))) {
            ...
            if (ua->sun_path[0]) {
                ...
                if (s->pst_lo_nodeid) {
                    ...
                    if (!(nma = (char *)malloc((MALLOC_S)len))) {
                        (void)fprintf(stderr, "%s: no unix nma space(1): PID %ld, FD %s",
                                      ProgramName, (long)CurrentLocalProc->pid,
                                      CurrentLocalFile->fd);
                    }
                    (void)snpf(nma, len, "%s", buf);
                    CurrentLocalFile->name_append = nma;
                }
                /*
                     ...
                     * preserved.
                     */
                if ((nau = read_det(&f->psf_fid, f->psf_hi_fileid, f->psf_lo_fileid,
                    ...
                    if (nma) {
                        ...
                        CurrentLocalFile->name_append = (char *)NULL;
                    }
                    if (s->pst_lo_nodeid) {
                    ...
                    if (err || s->pst_peer_lo_nodeid) {
                        (void)snpf(buf, sizeof(buf), "%s%s%s%s%s%s%s", err ? "(Error: " : "",
                        ...
                        len = strlen(buf) + 1;
                        if (!(nma = (char *)malloc((MALLOC_S)len))) {
                            (void)fprintf(stderr, "%s: no unix nma space(2): PID %ld, FD %s",
                                          ProgramName, (long)CurrentLocalProc->pid,
                                          CurrentLocalFile->fd);
                        }
                        (void)snpf(nma, len, "%s", buf);
        ...
        break;
    default:
        (void)snpf(NameChars, NameCharsLength, "unsupported family: AF_%d", s->pst_family);
    }
    if (NameChars[0])
        enter_nm(NameChars);
}
 ...
 */
    ...
    if ((nsr = pstat_getstream(s, sz, (size_t)nsn, 0, &f->psf_fid)) < 1) {
        if (nsn) {
            (void)snpf(NameChars, NameCharsLength, "can't read %d stream structures%s%s", nsn,
 ...
 */
    if (OptSize) {
        if (CurrentLocalFile->access == 'r') {
            ...
            CurrentLocalFile->sz =
                (SZOFFTYPE)s[hx].val.head.pst_rbytes + (SZOFFTYPE)s[hx].val.head.pst_wbytes;
            CurrentLocalFile->sz_def = 1;
        }
    }
    /*
    ...
    if (s[hx].val.head.pst_flag & PS_STR_ISACLONE) {
        if (HaveCloneMaj && (CloneMaj == s[hx].val.head.pst_dev_major)) {
            for (cl = Clone; cl; cl = cl->next) {
                if (GET_MIN_DEV(DeviceTable[cl->dx].rdev) == s[hx].val.head.pst_dev_minor) {
                    dp = &DeviceTable[cl->dx];
                    break;
                }
            }
        } else {
            rdev = makedev(s[hx].val.head.pst_dev_major, s[hx].val.head.pst_dev_minor);
            dp = lkupdev(&DeviceOfDev, &rdev, 0, 1);
        }
        rdev = makedev(s[hx].val.head.pst_dev_minor, s[hx].val.head.pst_dev_seq);
    } else {
        rdev = makedev(s[hx].val.head.pst_dev_major, s[hx].val.head.pst_dev_minor);
        dp = lkupdev(&DeviceOfDev, &rdev, 0, 1);
    }
 ...
 */
    if ((f->psf_subtype == PS_SUBTYPE_CHARDEV) || (f->psf_subtype == PS_SUBTYPE_BLKDEV))
        i = 1;
    else
        i = 0;
...
struct pst_socket *read_sock(struct pst_fileinfo2 *f) {
    static struct pst_socket s;

    errno = 0;
    if (f) {
        if (pstat_getsocket(&s, sizeof(s), &f->psf_fid) > 0 &&
            f->psf_hi_fileid == s.pst_hi_fileid && f->psf_lo_fileid == s.pst_lo_fileid &&
            f->psf_hi_nodeid == s.pst_hi_nodeid && f->psf_lo_nodeid == s.pst_lo_nodeid)
            return (&s);
    }
