...
static struct ax25sin *check_ax25(INODETYPE i) {
    struct ax25sin *ap;
    int h;
...
static struct ipxsin *check_ipx(INODETYPE i) {
    int h;
    struct ipxsin *ip;
...
static struct nlksin *check_netlink(INODETYPE i) {
    int h;
    struct nlksin *lp;
...
static struct packin *check_pack(INODETYPE i) {
    int h;
    struct packin *pp;
...
static struct rawsin *check_raw(INODETYPE i) {
    int h;
    struct rawsin *rp;
...
static struct tcp_udp *check_tcpudp(INODETYPE i, char **p) {
    int h;
    struct tcp_udp *tp;
...
static struct rawsin *check_raw6(INODETYPE i) {
    int h;
    struct rawsin *rp;
...
static struct tcp_udp6 *check_tcpudp6(INODETYPE i, char **p) {
    int h;
    struct tcp_udp6 *tp6;
...
static struct uxsin *check_unix(INODETYPE i) {
    int h;
    struct uxsin *up;
 ...
 */
    if (AX25sin) {
        ...
        if (!AX25sin) {
            (void)fprintf(stderr, "%s: can't allocate %d AX25 hash pointer bytes\n", ProgramName,
 ...
 */
    ...
    while (fgets(buf, sizeof(buf) - 1, as)) {
         ...
         */
        ep = (char *)NULL;
        if (!fp[23] || !*fp[23] || (inode = strtoull(fp[23], &ep, 0)) == ULONG_MAX || !ep || *ep)
            continue;
        ...
        ep = (char *)NULL;
        if (!fp[21] || !*fp[21] || (sq = strtoul(fp[21], &ep, 0)) == ULONG_MAX || !ep || *ep)
            continue;
        sqs = (unsigned char)1;
        ep = (char *)NULL;
        if (!fp[22] || !*fp[22] || (rq = strtoul(fp[22], &ep, 0)) == ULONG_MAX || !ep || *ep)
            continue;
        rqs = (unsigned char)1;
        ep = (char *)NULL;
        if (!fp[4] || !*fp[4] || (state = strtoul(fp[4], &ep, 0)) == ULONG_MAX || !ep || *ep)
            continue;
        /*
        ...
        if (!fp[3] || !*fp[3])
            ...
            if (!(da = (char *)malloc(len + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d destination AX25 addr bytes: %s\n",
        ...
        if (!fp[2] || !*fp[2])
            ...
            if (!(sa = (char *)malloc(len + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d source AX25 address bytes: %s\n",
        ...
        if (!fp[1] || !*fp[1])
            ...
            if (!(dev_ch = (char *)malloc(len + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d destination AX25 dev bytes: %s\n",
        ...
        if (!(ap = (struct ax25sin *)malloc(sizeof(struct ax25sin)))) {
            (void)fprintf(stderr, "%s: can't allocate %d byte ax25sin structure\n", ProgramName,
 ...
 */
    if (Ipxsin) {
        ...
        if (!Ipxsin) {
            (void)fprintf(stderr, "%s: can't allocate %d IPX hash pointer bytes\n", ProgramName,
    ...
    while (fgets(buf, sizeof(buf) - 1, xs)) {
        ...
        if (fl) {
            ...
            if (!fp[0] || strcmp(fp[0], "Local_Address") || !fp[1] ||
                strcmp(fp[1], "Remote_Address") || !fp[2] || strcmp(fp[2], "Tx_Queue") || !fp[3] ||
                strcmp(fp[3], "Rx_Queue") || !fp[4] || strcmp(fp[4], "State") || !fp[5] ||
                strcmp(fp[5], "Uid") || !fp[6] || strcmp(fp[6], "Inode")) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: WARNING: unsupported format: %s\n", ProgramName, p);
                }
                break;
            }
            fl = 0;
         ...
         */
        ep = (char *)NULL;
        if (!fp[2] || !*fp[2] || (txq = strtoul(fp[2], &ep, 16)) == ULONG_MAX || !ep || *ep)
            continue;
        ep = (char *)NULL;
        if (!fp[3] || !*fp[3] || (rxq = strtoul(fp[3], &ep, 16)) == ULONG_MAX || !ep || *ep)
            continue;
        ep = (char *)NULL;
        if (!fp[4] || !*fp[4] || (state = strtoul(fp[4], &ep, 16)) == ULONG_MAX || !ep || *ep)
            continue;
        /*
        ...
        if (!fp[0] || !*fp[0] || strcmp(fp[0], "Not_Connected") == 0)
            ...
            if (!(la = (char *)malloc(len + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d local IPX address bytes: %s\n",
        ...
        if (!fp[1] || !*fp[1] || strcmp(fp[1], "Not_Connected") == 0)
            ...
            if (!(ra = (char *)malloc(len + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d remote IPX address bytes: %s\n",
                              ProgramName, (int)(len + 1), fp[1]);
                Exit(1);
            }
        ...
        if (!(ip = (struct ipxsin *)malloc(sizeof(struct ipxsin)))) {
            (void)fprintf(stderr, "%s: can't allocate %d byte ipxsin structure\n", ProgramName,
...
static void get_netlink(char *p) {
 ...
 */
    if (Nlksin) {
        ...
        if (!Nlksin) {
            (void)fprintf(stderr, "%s: can't allocate %d netlink hash pointer bytes\n", ProgramName,
 ...
 */
    ...
    while (fgets(buf, sizeof(buf) - 1, xs)) {
        ...
        if (fr) {
             ...
             * Check the column labels in the first line.
             */
            if (!fp[1] || strcmp(fp[1], "Eth") || !fp[9] || strcmp(fp[9], "Inode")) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: WARNING: unsupported format: %s\n", ProgramName, p);
                }
                break;
         ...
         */
        ep = (char *)NULL;
        if (!fp[9] || !*fp[9] || (inode = strtoull(fp[9], &ep, 0)) == ULONG_MAX || !ep || *ep)
            continue;
        h = INOHASH(inode);
        for (lp = Nlksin[h]; lp; lp = lp->next) {
            if (inode == lp->inode)
        ...
        if (!(lp = (struct nlksin *)malloc(sizeof(struct nlksin)))) {
            (void)fprintf(stderr, "%s: can't allocate %d byte Netlink structure\n", ProgramName,
 ...
 */
    if (Packin) {
        ...
        if (!Packin) {
            (void)fprintf(stderr, "%s: can't allocate %d packet hash pointer bytes\n", ProgramName,
 ...
 */
    ...
    while (fgets(buf, sizeof(buf) - 1, xs)) {
        ...
        if (fl) {
             ...
             * Check the column labels in the first line.
             */
            if (!fp[2] || strcmp(fp[2], "Type") || !fp[3] || strcmp(fp[3], "Proto") || !fp[8] ||
                strcmp(fp[8], "Inode")) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: WARNING: unsupported format: %s\n", ProgramName, p);
                }
         ...
         */
        ep = (char *)NULL;
        if (!fp[8] || !*fp[8] || (inode = strtoull(fp[8], &ep, 0)) == ULONG_MAX || !ep || *ep)
            continue;
        h = INOHASH(inode);
        for (pp = Packin[h]; pp; pp = pp->next) {
            if (inode == pp->inode)
        ...
        if (!(pp = (struct packin *)malloc(sizeof(struct packin)))) {
            (void)fprintf(stderr, "%s: can't allocate %d byte packet structure\n", ProgramName,
 ...
 */
    if (Rawsin) {
        ...
        if (!Rawsin) {
            (void)fprintf(stderr, "%s: can't allocate %d raw hash pointer bytes\n", ProgramName,
    ...
    while (fgets(buf, sizeof(buf) - 1, xs)) {
        ...
        if (nf == 12) {
            ...
            if (!fp[1] || strcmp(fp[1], "local_address") || !fp[2] ||
                strcmp(fp[2], "rem_address") || !fp[3] || strcmp(fp[3], "st") || !fp[11] ||
                strcmp(fp[11], "inode")) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: WARNING: unsupported format: %s\n", ProgramName, p);
                }
        ...
        if (!fp[1] || !*fp[1] || (lal = strlen(fp[1])) < 1) {
            ...
            if (!(la = (char *)malloc(lal + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d local raw address bytes: %s\n",
        ...
        if (!fp[2] || !*fp[2] || (ral = strlen(fp[2])) < 1) {
            ...
            if (!(ra = (char *)malloc(ral + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d remote raw address bytes: %s\n",
        ...
        if (!fp[3] || !*fp[3] || (spl = strlen(fp[3])) < 1) {
            ...
            if (!(sp = (char *)malloc(spl + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d remote raw state bytes: %s\n",
        ...
        if (!(rp = (struct rawsin *)malloc(sizeof(struct rawsin)))) {
            (void)fprintf(stderr, "%s: can't allocate %d byte rawsin structure\n", ProgramName,
...
static void get_tcpudp(p, pr, clr) char *p; /* /proc/net/{tcp,udp} path */
 ...
 */
        ...
        if ((fs = fopen(SockStatPath, "r"))) {
            while (fgets(buf, sizeof(buf) - 1, fs)) {
                if (get_fields(buf, (char *)NULL, &fp, (int *)NULL, 0) != 3)
                    continue;
                if (!fp[0] || strcmp(fp[0], "sockets:") || !fp[1] || strcmp(fp[1], "used") ||
                    !fp[2] || !*fp[2])
                    continue;
                if ((h = atoi(fp[2])) < 1)
                    h = INOBUCKS;
        ...
        if (!(TcpUdp = (struct tcp_udp **)calloc(TcpUdp_bucks, sizeof(struct tcp_udp *)))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for TCP&UDP hash buckets\n",
 ...
 */
    ...
    nf = 12;
    while (fgets(buf, sizeof(buf) - 1, fs)) {
        if (get_fields(buf, (nf == 12) ? (char *)NULL : ":", &fp, (int *)NULL, 0) < nf)
            continue;
        if (nf == 12) {
            if (!fp[1] || strcmp(fp[1], "local_address") || !fp[2] ||
                strcmp(fp[2], "rem_address") || !fp[3] || strcmp(fp[3], "st") || !fp[4] ||
                strcmp(fp[4], "tx_queue") || !fp[5] || strcmp(fp[5], "rx_queue") || !fp[11] ||
                strcmp(fp[11], "inode")) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: WARNING: unsupported format: %s\n", ProgramName, p);
                }
        ...
        if (!fp[2] || !*fp[2] || (lport = strtoul(fp[2], &ep, 16)) == ULONG_MAX || !ep || *ep)
            continue;
        ep = (char *)NULL;
        if (!fp[3] || !*fp[3] || (faddr = strtoul(fp[3], &ep, 16)) == ULONG_MAX || !ep || *ep)
            continue;
        ep = (char *)NULL;
        if (!fp[4] || !*fp[4] || (fport = strtoul(fp[4], &ep, 16)) == ULONG_MAX || !ep || *ep)
            continue;
        /*
        ...
        if (!(tp = (struct tcp_udp *)malloc(sizeof(struct tcp_udp)))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for tcp_udp struct\n", ProgramName,
 ...
 */
    if (Rawsin6) {
        ...
        if (!Rawsin6) {
            (void)fprintf(stderr, "%s: can't allocate %d raw6 hash pointer bytes\n", ProgramName,
                          (int)(INOBUCKS * sizeof(struct rawsin *)));
            Exit(1);
        }
    }
 ...
 */
    ...
    while (fgets(buf, sizeof(buf) - 1, xs)) {
         ...
         */
            if (!fp[1] || strcmp(fp[1], "local_address") || !fp[2] ||
                strcmp(fp[2], "remote_address") || !fp[3] || strcmp(fp[3], "st") || !fp[11] ||
                strcmp(fp[11], "inode")) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: WARNING: unsupported format: %s\n", ProgramName, p);
                }
     ...
     */
        if (!fp[1] || !*fp[1] || (lal = strlen(fp[1])) < 1) {
            ...
            if (!(la = (char *)malloc(lal + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d local raw6 address bytes: %s\n",
                              ProgramName, (int)(lal + 1), fp[1]);
                Exit(1);
            }
            (void)snpf(la, lal + 1, "%s", fp[1]);
        ...
        if (!fp[2] || !*fp[2] || (ral = strlen(fp[2])) < 1) {
            ...
            if (!(ra = (char *)malloc(ral + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d remote raw6 address bytes: %s\n",
                              ProgramName, (int)(ral + 1), fp[2]);
                Exit(1);
            }
            (void)snpf(ra, ral + 1, "%s", fp[2]);
        ...
        if (!fp[3] || !*fp[3] || (spl = strlen(fp[3])) < 1) {
            ...
            if (!(sp = (char *)malloc(spl + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d remote raw6 state bytes: %s\n",
                              ProgramName, (int)(spl + 1), fp[2]);
                Exit(1);
            }
            (void)snpf(sp, spl + 1, "%s", fp[3]);
     ...
     */
        if (!(rp = (struct rawsin *)malloc(sizeof(struct rawsin)))) {
            (void)fprintf(stderr, "%s: can't allocate %d byte rawsin structure for IPv6\n",
...
static void get_tcpudp6(char *p, int pr, int clr) {
    char buf[MAXPATHLEN], *ep, **fp;
     ...
     */
        ...
        if ((fs = fopen(SockStatPath6, "r"))) {
            while (fgets(buf, sizeof(buf) - 1, fs)) {
                if (get_fields(buf, (char *)NULL, &fp, (int *)NULL, 0) != 3)
                    continue;
                if (!fp[0] || !fp[1] || strcmp(fp[1], "inuse") || !fp[2] || !*fp[2])
                    continue;
                if (!strcmp(fp[0], "TCP6:")) {
                    nf |= 1;
                    if ((h = atoi(fp[2])) < 1)
        ...
        if (!(TcpUdp6 = (struct tcp_udp6 **)calloc(TcpUdp6_bucks, sizeof(struct tcp_udp6 *)))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for TCP6&UDP6 hash buckets\n",
 ...
 */
    ...
    while (fgets(buf, sizeof(buf) - 1, fs)) {
        ...
        if (nf == 12) {
            if (!fp[1] || strcmp(fp[1], "local_address") || !fp[2] ||
                strcmp(fp[2], "remote_address") || !fp[3] || strcmp(fp[3], "st") || !fp[4] ||
                strcmp(fp[4], "tx_queue") || !fp[5] || strcmp(fp[5], "rx_queue") || !fp[11] ||
                strcmp(fp[11], "inode")) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: WARNING: unsupported format: %s\n", ProgramName, p);
     ...
     */
        if (!(tp6 = (struct tcp_udp6 *)malloc(sizeof(struct tcp_udp6)))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for tcp_udp6 struct\n", ProgramName,
 ...
 */
    if (Uxsin) {
        ...
        if (!Uxsin) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for Unix socket info\n", ProgramName,
                          (int)(INOBUCKS * sizeof(struct uxsin *)));
        }
    }
    /*
 ...
 */
    ...
    while (fgets(buf, sizeof(buf) - 1, us)) {
        ...
        if (fl) {
             ...
             */
            if (!fp[0] || strcmp(fp[0], "Num") || !fp[1] || strcmp(fp[1], "RefCount") || !fp[2] ||
                strcmp(fp[2], "Protocol") || !fp[3] || strcmp(fp[3], "Flags") || !fp[4] ||
                strcmp(fp[4], "Type") || !fp[5] || strcmp(fp[5], "St") || !fp[6] ||
                strcmp(fp[6], "Inode") || nf < 8 || !fp[7] || strcmp(fp[7], "Path")) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: WARNING: unsupported format: %s\n", ProgramName, p);
                }
                break;
        ...
        if (!fp[0] || !*fp[0])
            ...
            if (!(pcb = (char *)malloc(len + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d bytes for UNIX PCB: %s\n", ProgramName,
                              (int)(len + 1), fp[0]);
                Exit(1);
            }
        ...
        if (nf >= 8 && fp[7] && *fp[7] && (len = strlen(fp[7]))) {
            if (!(path = (char *)malloc(len + 1))) {
                (void)fprintf(stderr, "%s: can't allocate %d bytes for UNIX path \"%s\"\n",
        ...
        if (!(up = (struct uxsin *)malloc(sizeof(struct uxsin)))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for uxsin struct\n", ProgramName,
                          (int)sizeof(struct uxsin));
            Exit(1);
        }
        up->inode = inode;
        up->pcb = pcb;
 ...
 */
    for (i = 0, len = strlen(as); (i < 4) && (len >= 8); as += 8, i++, len -= 8) {
        (void)strncpy(buf, as, 8);
        buf[8] = '\0';
        ep = (char *)NULL;
        if ((ad->s6_addr32[i] = (uint32_t)strtoul(buf, &ep, 16)) == (uint32_t)UINT32_MAX || !ep ||
            *ep)
            break;
    }
...
static void print_ax25info(struct ax25sin *ap) {
    char *cp, pbuf[1024];
    int ds;
    ...
    if (ap->sa) {
        ...
        (void)snpf(&pbuf[pl], sizeof(pbuf) - pl, "%s%s%s ", ap->sa, ds ? "->" : "",
                   ds ? ap->da : "");
        pl = strlen(pbuf);
    }
    if (ap->sqs) {
    ...
    if ((ap->state >= 0) && (ap->state < NAX25ST))
        ...
        cp = NULL;
    (void)snpf(&pbuf[pl], sizeof(pbuf) - pl, "%s%s)", cp ? ", " : "", cp ? cp : "");
    pl = strlen(pbuf);
    if (!(cp = (char *)malloc(pl + 1))) {
        (void)fprintf(stderr, "%s: can't allocate %d bytes for AX25 sock state, PID: %d\n",
...
static void print_ipxinfo(struct ipxsin *ip) {
    char *cp, pbuf[256];
    MALLOC_S pl;

    if (CurrentLocalFile->name_append)
        return;
    (void)snpf(pbuf, sizeof(pbuf), "(Tx=%lx Rx=%lx State=%02x)", ip->txq, ip->rxq, ip->state);
    pl = strlen(pbuf);
    if (!(cp = (char *)malloc(pl + 1))) {
        (void)fprintf(stderr, "%s: can't allocate %d bytes for IPX sock state, PID: %d\n",
                      ProgramName, (int)(pl + 1), CurrentLocalProc->pid);
        Exit(1);
    }
    (void)snpf(cp, pl + 1, "%s", pbuf);
...
void print_tcptpi(int nl) {
    ...
    if ((OptTcpTpiInfo & TCPTPI_STATE) && CurrentLocalFile->lts.type == 0) {
        if (!TcpStateNames)
            (void)build_IPstates();
        if ((s = CurrentLocalFile->lts.state.val + TcpStateOffset) < 0 || s >= TcpNumStates) {
            (void)snpf(buf, sizeof(buf), "UNKNOWN_TCP_STATE_%d", CurrentLocalFile->lts.state.val);
            cp = buf;
        } else
            cp = TcpStateNames[s];
        if (cp) {
...
void process_proc_sock(p, s, ss, l, lss) char *p; /* node's readlink() path */
 ...
 */
        ...
        *cp = '\0';
        ...
        if (rp->sp && rp->spl) {
            ...
            if (nl > (len = ((cp == NameChars) ? 0 : 1) + 3 + rp->spl)) {
                (void)snpf(cp, nl, "%sst=%s", (cp == NameChars) ? "" : " ", rp->sp);
                cp += len;
                *cp = '\0';
                nl -= len;
            }
    ...
    if ((ss & SB_INO) && (pp = check_pack((INODETYPE)s->st_ino))) {
        ...
        (void)snpf(CurrentLocalFile->iproto, sizeof(CurrentLocalFile->iproto), "%.*s", IPROTOL - 1,
                   cp);
        CurrentLocalFile->inp_ty = 2;
        if (ss & SB_INO) {
            (void)snpf(tbuf, sizeof(tbuf), InodeFormatDecimal, (INODETYPE)s->st_ino);
            tbuf[sizeof(tbuf) - 1] = '\0';
            enter_dev_ch(tbuf);
        }
        if (NameChars[0])
    ...
    if ((ss & SB_INO) && (up = check_unix((INODETYPE)s->st_ino))) {
        ...
        if (SearchFileChain) {
            ...
            if (up->sb_def) {
                ...
                CurrentLocalFile->dev_def = CurrentLocalFile->inp_ty = CurrentLocalFile->rdev_def =
                    1;
            ...
            if (!f && (ss & SB_MODE)) {
                 ...
                 */
                if (is_file_named(path, ((s->st_mode & S_IFMT) == S_IFCHR)) ? 1 : 0) {
                    CurrentLocalFile->sel_flags |= SELNM;
                }
            }
        }
    ...
    if (!OptCrossoverExt && (ss & SB_INO) && (rp = check_raw6((INODETYPE)s->st_ino))) {
    ...
    if (!OptCrossoverExt && (ss & SB_INO) && (tp6 = check_tcpudp6((INODETYPE)s->st_ino, &pr))) {
     ...
     */
        ...
        CurrentLocalFile->lts.state.val = tp6->state;

#if defined(HASTCPTPIQ)
        CurrentLocalFile->lts.recv_queue = tp6->rxq;
        CurrentLocalFile->lts.send_queue = tp6->txq;
    ...
    if (!OptCrossoverExt && (ss & SB_INO) && (tp = check_tcpudp((INODETYPE)s->st_ino, &pr))) {
        ...
        CurrentLocalFile->lts.state.val = tp->state;

#if defined(HASTCPTPIQ)
        CurrentLocalFile->lts.recv_queue = tp->rxq;
        CurrentLocalFile->lts.send_queue = tp->txq;
        CurrentLocalFile->lts.recv_queue_st = CurrentLocalFile->lts.send_queue_st = 1;
#endif /* defined(HASTCPTPIQ) */
...
void set_net_paths(char *p, int pl) {
    int pathl;
