 ...
 */
    ...
    if (!(cs = (char *)malloc(ol + 1))) {
        (void)fprintf(stderr, "%s: can't allocate %d bytes for octal-escaping.\n", ProgramName,
                      ol + 1);
        Exit(1);
    }
    /*
    ...
    for (cx = ox = 0, cl = ol; ox < ol; ox++) {
        if (((c = (int)os[ox]) == (int)'\\') && ((ox + 3) < ol)) {
            ...
            for (tc = 0, tx = 1; os[ox + tx] && (tx < 4); tx++) {
                if (((int)os[ox + tx] < (int)'0') || ((int)os[ox + tx] > (int)'7')) {
        ...
        if (cx >= cl) {
            ...
            if (!(cs = (char *)realloc(cs, cl + 1))) {
                (void)fprintf(stderr, "%s: can't realloc %d bytes for octal-escaping.\n",
                              ProgramName, cl + 1);
                Exit(1);
            }
        }
					 ...
					 * values */
{
     ...
     */
         ...
         */
                if (!OptWarnings)
                    (void)fprintf(stderr, "%s: %s line %d: illegal device: \"%s\"\n", ProgramName,
                                  MountSupplementPath, ln, buf);
                err = 1;
                continue;
            }
         ...
         */
            if (!MSHash) {
                if (!(MSHash = (mntsup_t **)calloc(HASHMNT, sizeof(mntsup_t *)))) {
                    (void)fprintf(stderr, "%s: no space for mount supplement hash buckets\n",
                                  ProgramName);
                    Exit(1);
                }
            }
         ...
         */
                if (mp->dev != dev) {
                    (void)fprintf(stderr, "%s: %s line %d path duplicate of %d: \"%s\"\n",
         ...
         */
            if (!(mpn = (mntsup_t *)malloc(sizeof(mntsup_t)))) {
                (void)fprintf(stderr, "%s: no space for mount supplement entry: %d \"%s\"\n",
                              ProgramName, ln, buf);
                Exit(1);
            }
            if (!(mpn->dn = (char *)malloc(sz + 1))) {
                (void)fprintf(stderr, "%s: no space for mount supplement path: %d \"%s\"\n",
                              ProgramName, ln, buf);
                Exit(1);
            }
 ...
 */
    while (fgets(buf, sizeof(buf), ms)) {
        if (get_fields(buf, (char *)NULL, &fp, (int *)NULL, 0) < 3 || !fp[0] || !fp[1] || !fp[2])
            continue;
        /*
         ...
         */
            if ((MountSupplementState == 2) && MountSupplementPath) {
                ...
                if (getmntdev(dn, &sb, &ds) || !(ds & SB_DEV)) {
                    (void)fprintf(stderr, "%s: assuming dev=%#lx for %s from %s\n", ProgramName,
                                  (long)sb.st_dev, dn, MountSupplementPath);
                }
            } else {
                if (!ignstat)
                    continue;
        ...
        if (!(mp = (struct mounts *)malloc(sizeof(struct mounts)))) {
            (void)fprintf(stderr, "%s: can't allocate mounts struct for: ", ProgramName);
            safestrprt(dn, stderr, 1);
            Exit(1);
        }
        mp->dir = dn;
        ...
        if (ignrdl || (*dn != '/')) {
            if (!(ln = mkstrcpy(dn, (MALLOC_S *)NULL))) {
                (void)fprintf(stderr, "%s: can't allocate space for: ", ProgramName);
                safestrprt(dn, stderr, 1);
                Exit(1);
            }
            ignstat = 1;
