...
static unsigned char Dos2Unix[] = {
    /* derived from tounix[] in
...
int readbfslino(struct vnode *v, struct l_ino *i) {
    struct inode b;
    struct vfs kv;
...
int readcdfslino(struct vnode *v, struct l_ino *i) {
    cdfs_inode_t ci;
    TYPELOGSECSHIFT lss;
    KA_T ka;
    struct vfs kv;
 ...
 */
    ...
    if (!kread(ka, (char *)&lss, sizeof(lss))) {
        i->number = (INODETYPE)((ci.i_Fid.fid_SectNum << lss) + ci.i_Fid.fid_Offset);
        i->number_def = 1;
    }
    return (0);
}
...
int readdosfslino(struct vnode *v, struct l_ino *i) {
    struct dosfs_inode {
        int pad1[19];
        int device;
        int pad2[21];
 ...
 */
    ...
    if (di.name[0]) {
        if (!nm) {
            nml = (MALLOC_S)(strlen(DOS_NAME_PFX) + sizeof(di.name) + strlen(DOS_NAME_SFX) + 1);
            if (!(nm = (char *)malloc(nml))) {
                (void)fprintf(stderr, "%s: can't allocate %d bytes for DOS name\n", nml,
                              ProgramName);
                Exit(1);
            }
        }
     ...
     */
        ...
        if (sz) {
            (void)snpf(nm, nml, "%s%-*.*s%s", DOS_NAME_PFX, sz, sz, di.name, DOS_NAME_SFX);
            i->nm = nm;
        }
    }
    /*
