 ...
 */
    ...
    for (p = P, px = 0; px < Np; p++, px++) {
         ...
         */
        if (!p->p_execinfo || kread((KA_T)p->p_execinfo, (char *)&ex, sizeof(ex)))
            continue;
        /*
        ...
        if (is_cmd_excl(ex.ei_comm, &pss, &sf))
            continue;
        alloc_lproc(pid, pgid, (int)p->p_ppid, (UID_ARG)uid, ex.ei_comm, (int)pss, (int)sf);
        PrevLocalFile = NULL;
        /*
        ...
        for (f = fe, i = 0; i < nf; f++, i++) {
            if ((fa = (KA_T)f->fd_file) && (f->fd_status & FD_INUSE)) {
                ...
                if (f->fd_flag & FPOLLED) {
                    if (kread(fa, (char *)&plx, sizeof(plx)) || !(fa = (KA_T)plx.px_fp))
                        continue;
                }
#endif /* UNIXWAREV>=70103 */
 ...
 */
    if (get_Nl_value("ncdev", Drive_Nl, &v) < 0 || !v || kread((KA_T)v, (char *)&sz, sizeof(sz)) ||
        !sz)
        return (rv);
 ...
 */
    if (get_Nl_value("cdev", Drive_Nl, &v) < 0 || !v || kread((KA_T)v, (char *)cd, (int)len)) {
 ...
 */
    ...
    for (c = cd, i = 0; i < sz; c++, i++) {
        if (!c->d_name || kread((KA_T)c->d_name, buf, len) || strcmp(buf, "clone") != 0)
            continue;
        CloneMaj = i;
        HaveCloneMaj = rv = 1;
        break;
 ...
 */
    if (!get_clonemaj()) {
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: WARNING; can't identify major clone device number\n",
                          ProgramName);
    }
}
 ...
 */
    for (try = 0; try < PROCTRYLM; try++) {
        ...
        if (!Pract || kread((KA_T)Pract, (char *)&pa, sizeof(pa)) || !pa) {
            if (!OptWarnings)
                (void)fprintf(stderr, "%s: active proc chain ptr err; addr=%s, val=%s\n",
