...
void process_socket(char *pr, struct queue *q) {
...
int process_unix_sockstr(struct vnode *v, KA_T na) {
    int as;
 ...
 */
    if (!(sh = (KA_T)v->v_stream))
        return (0);
    if (readstdata(sh, &sd)) {
        (void)snpf(NameChars, NameCharsLength, "vnode at %s; can't read stream head at %s",
                   print_kptr(na, (char *)NULL, 0), print_kptr(sh, tbuf, sizeof(tbuf)));
        enter_nm(NameChars);
        return (1);
    }
    /*
 ...
 */
    ...
    if (i >= alct || !al[i].sockaddr.sun_path[0])
        return ((struct sockaddr_un *)NULL);
    al[i].sockaddr.sun_path[(int)(sizeof(al[i].sockaddr.sun_path) - 1)] = '\0';
