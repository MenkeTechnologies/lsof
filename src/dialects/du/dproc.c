 ...
 */
    for (p = Ps, px = 0, utp = &ut; px < Psn; p++, px++) {
        if (p->p_stat == 0 || p->p_stat == SZOMB)
            continue;
        if (OptProcessGroup) {
            if (!p->p_pgrp || kread((KA_T)p->p_pgrp, (char *)&pg, sizeof(pg)))
                continue;
            pgid = pg.pg_id;
        } else
            pgid = 0;
        if (p->p_rcred == NULL || kread((KA_T)p->p_rcred, (char *)&pcred, sizeof(pcred)))
            continue;
...
#if DUV < 30000
        if (!p->utask || kread((KA_T)p->utask, (char *)&ut, sizeof(ut)))
#else  /* DUV>=30000 */
        if (kread((KA_T)((char *)Pa[px] + sizeof(struct proc)), (char *)&ut, sizeof(ut)))
#endif /* DUV<30000 */
        ...
        if (is_cmd_excl(utp->u_comm, &pss, &sf))
            continue;
        alloc_lproc((int)p->p_pid, pgid, (int)p->p_ppid, (UID_ARG)uid, utp->u_comm, (int)pss,
                    (int)sf);
        ...
        for (i = j = 0; i <= utp->uu_file_state.uf_lastfile; i++) {
            if (i < NOFILE_IN_U) {
                ...
                if (!j) {
                    b = (MALLOC_S)(utp->uu_file_state.uf_of_count * sizeof(struct file *));
                    if (b > nufb) {
                        if (!ufb)
                            ufb = (struct file **)malloc(b);
                        else
                            ufb = (struct file **)realloc((MALLOC_P *)ufb, b);
                        if (!ufb) {
                    ...
                    if (kread((KA_T)utp->uu_file_state.uf_ofile_of, (char *)ufb, b))
                        break;

#if defined(HASFSTRUCT) && DUV >= 40000
                    if (OptFileStructValues & FSV_FILE_FLAGS) {
                        b = (MALLOC_S)(utp->uu_file_state.uf_of_count * sizeof(char));
                        if (b > pofb) {
                            if (!pof)
                                pof = (char *)malloc(b);
                            else
                            ...
                            if (!pof) {
                                (void)fprintf(stderr, "%s: PID %d: no file flags space\n",
                                              ProgramName, CurrentLocalProc->pid);
                                Exit(1);
                            }
                            pofb = b;
                        }
                        if (kread((KA_T)utp->uu_file_state.uf_pofile_of, pof, b))
                            zeromem(pof, b);
                    }
#endif /* defined(HASFSTRUCT) && DUV>=40000 */
                }
 ...
 */
    ...
    if (rv == -1) {
        (void)fprintf(stderr, "%s: can't read kernel name list from %s: %s\n", ProgramName,
                      NamelistFilePath ? NamelistFilePath : "knlist(3)", strerror(errno));
        Exit(1);
    }

#if DUV < 30000
    if (get_Nl_value("proc", Drive_Nl, &v) < 0 || !v || kread(v, (char *)&Kp, sizeof(Kp)) ||
        get_Nl_value("nproc", Drive_Nl, &v) < 0 || !v || kread(v, (char *)&Np, sizeof(Np)))
#else  /* DUV>=30000 */
    if (get_Nl_value("npid", Drive_Nl, &v) < 0 || !v || kread(v, (char *)&Np, sizeof(Np)) ||
        get_Nl_value("pidt", Drive_Nl, &v) < 0 || !v || kread(v, (char *)&Pidtab, sizeof(Pidtab)))
#endif /* DUV<30000 */
    ...
    if (get_Nl_value("vnmaxp", Drive_Nl, &v) < 0 || !v || kread(v, (char *)&Vnmxp, sizeof(Vnmxp))) {
        (void)fprintf(stderr, "%s: can't determine vnode length\n", ProgramName);
        Exit(1);
    }
    if (get_Nl_value("cldev", Drive_Nl, &v) < 0 || !v || kread(v, (char *)&dev, sizeof(dev))) {
        if (!OptWarnings)
 ...
 */
    len = 0;
    if ((rv = getsysinfo(GSI_BOOTEDFILE, &buf[1], sizeof(buf) - 1, &len, (char *)NULL)) != 1) {
        if (rv < 0) {
 ...
 */
    if (!t.map || kread((KA_T)t.map, (char *)&vmm, sizeof(vmm)))
        return;
    if (!vmm.vm_is_mainmap)
        return;

#if defined(VM_SKIPLIST)
    for (i = 0, ka = (KA_T)vmm.vm_links.vml_sl_next[0]; i < vmm.vm_nentries && ka != (KA_T)t.map;
         i++, ka = (KA_T)vmme.vme_links.vml_sl_next[0])
#else  /* !defined(VM_SKIPLIST) */
    for (i = 0, ka = (KA_T)vmm.vm_links.next; i < vmm.vm_nentries && ka != (KA_T)t.map;
...

    {
        ...
        if (kread(ka, (char *)&vmme, sizeof(vmme)))
            return;
        if (!(kb = (KA_T)vmme.vme_uobject.vm_object) || kread(kb, (char *)&vmo, sizeof(vmo)))
            continue;
        /*
        ...
        case OT_ANON:
            ...
            if (kread(kb, (char *)&vmao, sizeof(vmao)))
                break;
            if (!vmao.ao_bobject || kread((KA_T)vmao.ao_bobject, (char *)&vmo, sizeof(vmo)))
                break;

#if DUV < 40000
            if (vmo.ob_type != OT_VP || kread((KA_T)vmao.ao_bobject, (char *)&vpo, sizeof(vpo)))
                break;
            enter_vn_text((KA_T)vpo.vo_vp, &n);
#else /* DUV>=40000 */
            if (vmo.ob_type != OT_UBC || kread((KA_T)vmao.ao_bobject, (char *)&vpo, sizeof(vpo)))
                break;
#if DUV >= 50000
        ...
        case OT_SEG:
            for (kb = (KA_T)vmme.vme_seg; kb; kb = (KA_T)vms.seg_vnext) {
                if (kread(kb, (char *)&vms, sizeof(vms)))
                    break;
                if (!vms.seg_vop || kread((KA_T)vms.seg_vop, (char *)&vmo, sizeof(vmo)))
                    continue;
...
static void read_proc() {
    ...
    if (!Ps) {
        ...
        if (!(Pa = (KA_T *)malloc(len))) {
            (void)fprintf(stderr, "%s: no proc address table space (%d bytes)\n", ProgramName, len);
            Exit(1);
        }
#endif /* DUV>=30000 */
        ...
        for (p = Ps, Psn = px = 0; px < Np; px++) {
            ...
            if (Psn >= ap) {
                ...
                if (!(Ps = (struct proc *)realloc((MALLOC_P *)Ps, len))) {
                    (void)fprintf(stderr, "%s: no more proc table space (%d bytes)\n", ProgramName,
                                  len);
                    Exit(1);
                }
                p = &Ps[Psn];
                ...
                if (!(Pa = (KA_T *)realloc((MALLOC_P *)Pa, len))) {
                    (void)fprintf(stderr, "%s: no more proc address table space (%d bytes)\n",
                                  ProgramName, len);
                    Exit(1);
                }
#endif /* DUV>=30000 */
            ...
            if (kread(pa, (char *)&pe, sizeof(struct pid_entry)))
                continue;
            if ((pa = (KA_T)pe.pe_proc) == NULL || kread(pa, (char *)p, sizeof(struct proc)))
                continue;
 ...
 */
    ...
    if (Psn < Np && !RepeatTime) {
        ...
        if (!(Ps = (struct proc *)realloc((MALLOC_P *)Ps, len))) {
            (void)fprintf(stderr, "%s: can't reduce proc table to %d bytes\n", ProgramName, len);
            Exit(1);
        }
        ...
        if (!(Pa = (KA_T *)realloc((MALLOC_P *)Pa, len))) {
            (void)fprintf(stderr, "%s: can't reduce proc address table to %d bytes\n", ProgramName,
                          len);
            Exit(1);
        }
#endif /* DUV>=30000 */
...
static KA_T vpo2vp(struct vm_ubc_object *vpo) {
...
static struct l_nch *ncache_addr(unsigned long id) {
...
static int ncache_ckrootid(KA_T na, unsigned long id) {
 ...
 */
     ...
     */
            rv = frvp;
        } else if ((nops && (nops == (KA_T)m.m_op)) || (n3ops && (n3ops == (KA_T)m.m_op))) {
 ...
 */
    if (niu >= nia) {
        ...
        if (!ic) {
            (void)fprintf(stderr, "%s: no space for root node VPID table\n", ProgramName);
            Exit(1);
        }
        nia += 10;
    }
 ...
 */
    if (kread((KA_T)na, (char *)&v, sizeof(v)) || v.v_type != VDIR || !(v.v_flag & VROOT)) {
 ...
 */
    if (ncn >= nca) {
        ...
        if (!nc) {
            (void)fprintf(stderr, "%s: no space for root node address table\n", ProgramName);
            Exit(1);
        }
        nca += 10;
    }
...
void ncache_load() {
     ...
     */
        ka = (KA_T)0;
        if (get_Nl_value("ncpus", (struct drive_Nl *)NULL, &ka) < 0 || !ka ||
            kread(ka, (char *)&ncpus, sizeof(ncpus))) {
            if (!OptWarnings)
                (void)fprintf(stderr, "%s: WARNING: can't read processor count: %s\n", ProgramName,
        ...
        if (ncpus < 1) {
            if (!OptWarnings)
                (void)fprintf(stderr, "%s: WARNING: processor count: %d\n", ProgramName, ncpus);
            ncl = nchsz = ncpc = ncpus = 0;
            return;
        }
        /*
     ...
     */
        ka = (KA_T)0;
        if (get_Nl_value("procptr", (struct drive_Nl *)NULL, &ka) < 0 || !ka ||
            kread(ka, (char *)&ka, sizeof(ka)) || !ka) {
            if (!OptWarnings)
                (void)fprintf(stderr, "%s: WARNING: per processor table address: %s\n", ProgramName,
                              print_kptr(ka, (char *)NULL, 0));
            ncl = nchsz = ncpc = ncpus = 0;
            return;
        }
     ...
     */
        ...
        if (!(pp = (KA_T *)malloc((MALLOC_S)len))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for processor addresses\n",
                          ProgramName, len);
            Exit(1);
        }
        if (kread(ka, (char *)pp, len)) {
            if (!OptWarnings)
                (void)fprintf(stderr, "%s: WARNING: can't read processor addresses: %s\n",
        ...
        for (i = 0; i < ncpus; i++) {
            if (pp[i])
                pp[i] = (KA_T)((char *)pp[i] + offsetof(struct processor, namecache));
        }
        /*
     * Get the per-processor nchash size.
     */
        ka = (KA_T)0;
        if (get_Nl_value("nchsz", (struct drive_Nl *)NULL, &ka) < 0 || !ka ||
            kread((KA_T)ka, (char *)&nchsz, sizeof(nchsz))) {
            if (!OptWarnings)
                (void)fprintf(stderr, "%s: WARNING: processor nchash count address: %s\n",
                              ProgramName, print_kptr(ka, (char *)NULL, 0));
            ncl = nchsz = ncpc = ncpus = 0;
            return;
        }
        if (nchsz < 1) {
            if (!OptWarnings)
                (void)fprintf(stderr, "%s: WARNING: bad per processor nchash count: %d\n",
                              ProgramName, nchsz);
            nchsz = ncpus = 1;
            return;
        }
     ...
     */
        ...
        if (!(nc = (struct namecache *)malloc((MALLOC_S)len))) {
            (void)fprintf(stderr, "%s: no space for %d namecache entries (%d bytes)\n", ProgramName,
                          ncpc * ncpus, len);
            Exit(1);
        }
    } else {
 ...
 */
    ...
    if (!(Nchash = (struct l_nch **)calloc(i, sizeof(struct l_nch *)))) {
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: no space for %d byte name cache hash buckets\n", ProgramName,
                          (int)(i * sizeof(struct l_nch *)));
        Exit(1);
    }
    /*
