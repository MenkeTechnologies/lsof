 ...
 */
    if (HaveSockdev && (i.i_ftype & IFMT) == IFCHR && GET_MAJ_DEV(i.i_rdev) == Sockdev) {
        ...
        if (i.i_sptr) {
            ...
            if ((dp = finddev(&CurrentLocalFile->dev, &CurrentLocalFile->rdev, 1))) {
                ...
                if ((cp = strrchr(dp->name, '/')))
                    cp++;
            } else if ((j = GET_MAJ_DEV(i.i_rdev)) < Cdevcnt && (cp = Cdevsw[j])) {
            ...
            if (p && pt >= 0) {
                ...
                if (pt == 1 && pcb.inp_ppcb) {
                    ...
                    /*
                         ...
                         */
                    if (kread((KA_T)pcb.inp_ppcb, (char *)&udp, sizeof(udp)) == 0) {

#if OSRV >= 500
                        if (udp.ud_lsin.sin_addr.s_addr != INADDR_ANY || udp.ud_lsin.sin_port != 0)
                            udpsl = 1;
                        else
                            udpsl = 0;
#endif /* OSRV>=500 */

                        if (udp.ud_fsin.sin_addr.s_addr != INADDR_ANY || udp.ud_fsin.sin_port != 0)
                            udpsf = 1;
                        else
                            udpsf = 0;
                    }
...
#if OSRV >= 500
                if (((struct in_addr *)la)->s_addr == INADDR_ANY && lp == 0 && udpsl) {
            ...
            if (ity) {
                ...
                NodeType = N_CHR;
                if (!finddev(&i.i_dev, &i.i_rdev, 0) && HaveEventMajor &&
                    GET_MAJ_DEV(i.i_rdev) == EventMajor)
                    (void)snpf(NameChars, NameCharsLength, "clone %d:/dev/event",
...
#if OSRV < 500
        CurrentLocalFile->dev = (dev_t)_makedev(~GET_MAJ_DEV(i.i_dev), GET_MIN_DEV(i.i_dev));
        CurrentLocalFile->rdev = (dev_t)_makedev(~GET_MAJ_DEV(i.i_rdev), GET_MIN_DEV(i.i_rdev));
#else  /* OSRV>=500 */
        CurrentLocalFile->dev = i.i_dev;
        CurrentLocalFile->rdev = i.i_rdev;
#endif /* OSRV<500 */
 ...
 */
    ...
    case IFNAM:
        if (i.i_rdev == S_INSEM)
            ...
            tn = "XNAM";
            (void)snpf(NameChars, NameCharsLength, "unknown Xenix special file type: %x", i.i_rdev);
        }
        break;
    case IFIFO:
        tn = "FIFO";
 ...
 */
    if (SearchFileChain &&
        is_file_named((char *)NULL,
                      ((type == IFCHR) || (type == IFBLK) || (type == IFNAM)) ? 1 : 0))
