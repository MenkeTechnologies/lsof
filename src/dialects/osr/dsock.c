...
void process_socket(struct inode *i) {
    char *cp;
    struct domain d;
 ...
 */
    if (!s.so_proto.pr_domain || kread((KA_T)s.so_proto.pr_domain, (char *)&d, sizeof(d))) {
 ...
 */
    ...
    case AF_INET:
        ...
        /*
             ...
             */
        if (s.so_stp && !readstdata((KA_T)s.so_stp, &sd) && !readsthead((KA_T)sd.sd_wrq, &sh))
            shs = 1;
        if (shs && sh.q_ptr) {
    ...
    case AF_UNIX:
        ...
        if (s.so_stp && !readstdata((KA_T)s.so_stp, &sd) && !readsthead((KA_T)sd.sd_wrq, &sh)) {
            ...
            if (ud.local_addr.sun_family == AF_UNIX) {
                ...
                ud.local_addr.sun_path[sizeof(ud.local_addr.sun_path) - 1] = '\0';
 ...
 */
    if (!Lbolt)
        return;
    if (Hz < 0 || kread((KA_T)Lbolt, (char *)&lbolt, sizeof(lbolt)) || tm >= lbolt ||
