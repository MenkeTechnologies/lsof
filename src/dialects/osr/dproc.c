...
static void Ckkv(char *d, char *er, char *ev, char *ea) {
 ...
 */
    if (!ua) {
        ...
        if (!(ua = (char *)malloc(ual))) {
            (void)fprintf(stderr, "%s: no space for %d byte user structure buffer\n", ProgramName,
                          ual);
            Exit(1);
        }
        u = (struct user *)ua;
 ...
 */
    for (pbc = px = 0; px < Var.v_proc; px++) {
        if (px >= pbc) {
            ...
            if (i > PROCBFRD)
                i = PROCBFRD;
            j = kread((KA_T)(Kp + (px * sizeof(struct proc))), pb, sizeof(struct proc) * i);
            pbc = px + i;
            p = (struct proc *)pb;
            if (j) {
                px += i;
        ...
        if (is_cmd_excl(u->u_comm, &pss, &sf))
            continue;
        alloc_lproc(pid, pgrp, (int)p->p_ppid, (UID_ARG)uid, u->u_comm, (int)pss, (int)sf);
 ...
 */
    if (get_Nl_value("cdev", Drive_Nl, &v[0]) < 0 || get_Nl_value("ncdev", Drive_Nl, &v[1]) < 0 ||
        !v[0] || !v[1] || kread(v[1], (char *)&Cdevcnt, sizeof(Cdevcnt)) || Cdevcnt < 1)
        return;
    /*
 ...
 */
    ...
    for (c = tmp, i = 0; i < Cdevcnt; c++, i++) {
        ...
        if (kread((KA_T)c->d_name, buf, j)) {
            (void)fprintf(stderr, "%s: WARNING: can't read name for cdevsw[%d]: %#x\n", ProgramName,
                          i, c->d_name);
            continue;
        }
        if (!buf[0])
 ...
 */
    v = (KA_T)0;
    if (get_Nl_value("pregpp", Drive_Nl, &v) < 0 || !v || kread(v, (char *)&Npp, sizeof(Npp)) ||
        Npp < 1) {
 ...
 */
    if (get_Nl_value("var", Drive_Nl, &v) < 0 || !v || kread((KA_T)v, (char *)&Var, sizeof(Var))) {
 ...
 */
    v = (KA_T)0;
    if (get_Nl_value("hz", Drive_Nl, &v) < 0 || !v || kread(v, (char *)&Hz, sizeof(Hz))) {
        if (!OptWarnings)
    ...
    if (get_Nl_value("lbolt", Drive_Nl, &Lbolt) < 0 || !v ||
        ...
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: WARNING: can't read lightning bolt timer from %s\n",
 ...
 */
    ...
    if (get_Nl_value("nxdm", Drive_Nl, &v) < 0 || !v ||
        ...
        (void)fprintf(stderr, "%s: bad extended device table size (%d) at %s.\n", ProgramName,
    ...
    v = (KA_T)0;
    if (get_Nl_value("xdm", Drive_Nl, &v) < 0 || !v || kread((KA_T)v, (char *)Xdevmap, len)) {
 ...
 */
    if (NlistTable) {
        (void)free((FREE_P *)NlistTable);
        NlistTable = (struct NLIST_TYPE *)NULL;
    }
    (void)build_Nl(Drive_Nl);
    if (nlist(p, NlistTable) < 0)
        return (0);
 ...
 */
...

    {
        ...
        if (i > 1000) {
            if (!OptWarnings)
                (void)fprintf(stderr, "%s: too many virtual address regions for PID %d\n",
        ...
        if (!Nc) {
            ...
            Nn += 10;
            if (!(Nc = (KA_T *)realloc((MALLOC_P *)Nc, (MALLOC_S)(Nn * sizeof(KA_T))))) {
 ...
 */
     ...
     */
        ...
        if (!(dnlc = (struct dnlc__cachent *)malloc(chl))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for DNLC chunk\n", ProgramName, chl);
            cha = 0;
            Exit(1);
        }
    }
 ...
 */
     ...
     */
        ...
        if (!(dtnc = (struct dtcachent *)malloc(kcl))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for DTFS name cache\n", ProgramName,
                          kcl);
            Exit(1);
        }
    }
 ...
 */
     ...
     */
        ...
        if (!(htnc = (struct htcachent *)malloc(kcl))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for HTFS name cache\n", ProgramName,
                          kcl);
            Exit(1);
        }
    }
...
static int LNC_enter(struct lnch *le, char *nm, int nl, char *fs) {
    ...
    if (LNC_csz >= LNC_asz) {
        ...
        if (!(LNC_nc = (struct lnch *)realloc(LNC_nc, len))) {
            (void)fprintf(stderr, "%s: no more space for %d byte local name cache: %s\n",
                          ProgramName, len, fs);
            Exit(1);
        }
    }
 ...
 */
    if (nl > MAXNSZ) {
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: WARNING: length for \"%s\" too large: %d\n", ProgramName, nm,
                          nl);
        nl = MAXNSZ;
    }
 ...
 */
    if (!LNC_hh) {
        ...
        if (!LNC_hh) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for name cache hash table\n",
 ...
 */
    for (i = 0, lc = LNC_nc; i < LNC_csz; i++, lc++) {
        ...
        for (f = 0, hlp = hl; hl; hlp = hl, hl = hl->next) {
            ...
            {
                if (lc->u.ld.dev != hl->u.ld.dev || lc->u.ld.inum != hl->u.ld.inum)
                    continue;
            }
         ...
         */
            for (hl = hh->hp[lc->type]; hl; hl = hl->next) {
                ...
                if (hl->u.ld.dev == lc->u.ld.dev && hl->u.ld.inum == lc->u.ld.inum)
                    hl->dup = 1;
            }
        }
    }
 ...
 */
     ...
     */
        ...
        if (!(nfnc = (struct ncache *)malloc(kcl))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for NFS name cache\n", ProgramName,
                          kcl);
            Exit(1);
        }
    }
 ...
 */
     ...
     */
        ...
        if (!(s5nc = (struct s5cachent *)malloc(kcl))) {
            (void)fprintf(stderr, "%s: can't allocate %d bytes for SYSV name cache\n", ProgramName,
                          kcl);
            Exit(1);
        }
    }
