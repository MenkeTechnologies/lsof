...
static void process_socket_common(struct socket_fdinfo *si) {
 ...
 */
    if (OptSize) {
        if (CurrentLocalFile->access == 'r')
            ...
            CurrentLocalFile->sz = (SZOFFTYPE)(si->psi.soi_rcv.sbi_cc + si->psi.soi_snd.sbi_cc);
 ...
 */
    ...
    case AF_INET6:
        ...
        if ((si->psi.soi_kind == SOCKINFO_TCP) && (TcpStateExcludeCount || TcpStateIncludeCount)) {
            int tsnx = (int)si->psi.soi_proto.pri_tcp.tcpsi_state + TcpStateOffset;
        ...
        /*
             ...
             */
        if (OptNetwork) {
            if (!OptNetworkType || ((OptNetworkType == 4) && (fam == AF_INET)) ||
                ((OptNetworkType == 6) && (fam == AF_INET6)))
                CurrentLocalFile->sel_flags |= SELNET;
        }
        printiproto(si->psi.soi_protocol);
        if ((si->psi.soi_kind == SOCKINFO_TCP) && si->psi.soi_proto.pri_tcp.tcpsi_tp) {
            enter_dev_ch(print_kptr((KA_T)si->psi.soi_proto.pri_tcp.tcpsi_tp, (char *)NULL, 0));
        } else
        ...
        if (fam == AF_INET) {
            ...
            if (si->psi.soi_kind == SOCKINFO_TCP) {
                ...
                /*
                     ...
                     */
                la = (unsigned char *)&si->psi.soi_proto.pri_tcp.tcpsi_ini.insi_laddr.ina_46
                         .i46a_addr4;
                lp = (int)ntohs(si->psi.soi_proto.pri_tcp.tcpsi_ini.insi_lport);
                fa = (unsigned char *)&si->psi.soi_proto.pri_tcp.tcpsi_ini.insi_faddr.ina_46
                         .i46a_addr4;
    ...
    case AF_UNIX:
        ...
        if (si->psi.soi_proto.pri_un.unsi_addr.ua_sun.sun_family != AF_UNIX) {
            if (si->psi.soi_proto.pri_un.unsi_addr.ua_sun.sun_family == AF_UNSPEC) {
                if (si->psi.soi_proto.pri_un.unsi_conn_pcb) {
                    (void)snpf(
                        NameChars, NameCharsLength, "->%s",
        ...
        if (si->psi.soi_proto.pri_un.unsi_addr.ua_sun.sun_path[0]) {
            ...
            if (si->psi.soi_proto.pri_un.unsi_addr.ua_sun.sun_path[0] && SearchFileChain &&
                is_file_named(si->psi.soi_proto.pri_un.unsi_addr.ua_sun.sun_path, 0))
                CurrentLocalFile->sel_flags |= SELNM;
            if (si->psi.soi_proto.pri_un.unsi_addr.ua_sun.sun_path[0] && !NameChars[0])
    ...
    case AF_NDRV:
        ...
        si->psi.soi_proto.pri_ndrv
            .ndrvsi_if_name[sizeof(si->psi.soi_proto.pri_ndrv.ndrvsi_if_name) - 1] = '\0';
        (void)snpf(NameChars, NameCharsLength, "-> %s%d", si->psi.soi_proto.pri_ndrv.ndrvsi_if_name,
...
void process_socket(int pid, int32_t fd) {
    int nb;
 ...
 */
    ...
    if (nb <= 0) {
        ...
        (void)fprintf(stderr, "%s: PID %d, FD %d: proc_pidfdinfo(PROC_PIDFDSOCKETINFO);\n",
                      ProgramName, pid, fd);
