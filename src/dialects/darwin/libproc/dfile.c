...
void enter_file_info(struct proc_fileinfo *pfi) {
    int f;
    /*
...
void enter_vnode_info(struct vnode_info_path *vip) {
    char buf[32], *cp;
    dev_t dev = 0;
    int devs = 0;
    struct mounts *mp;
 ...
 */
    if (SearchFileChain &&
        is_file_named(NULL, ((NodeType == N_CHR) || (NodeType == N_BLK) ? 1 : 0))) {
...
int print_v_path(struct lfile *lf) {
    if (lf->V_path) {
...
void process_atalk(int pid, int32_t fd) {
...
void process_fsevents(int pid, int32_t fd) {
...
void process_kqueue(int pid, int32_t fd) {
 ...
 */
    ...
    if (nb <= 0) {
        ...
        (void)fprintf(stderr, "%s: PID %d, FD %d; proc_pidfdinfo(PROC_PIDFDKQUEUEINFO);\n",
                      ProgramName, pid, fd);
 ...
 */
    (void)snpf(NameChars, NameCharsLength, "count=%" SZOFFPSPEC "u, state=%#x",
               (SZOFFTYPE)kq.kqueueinfo.kq_stat.vst_size, kq.kqueueinfo.kq_state);
    enter_nm(NameChars);
}
...
static void process_pipe_common(struct pipe_fdinfo *pi) {
    char dev_ch[32], *ep;
    size_t sz;
 ...
 */
    if (pi->pipeinfo.pipe_stat.vst_size) {
        ep = endnm(&sz);
        (void)snpf(ep, sz, ", cnt=%" SZOFFPSPEC "u", (SZOFFTYPE)pi->pipeinfo.pipe_stat.vst_size);
    }
}

void process_pipe(int pid, int32_t fd) {
    int nb;
 ...
 */
    ...
    if (nb <= 0) {
        ...
        (void)fprintf(stderr, "%s: PID %d, FD %d; proc_pidfdinfo(PROC_PIDFDPIPEINFO);\n",
                      ProgramName, pid, fd);
        (void)fprintf(stderr, "      too few bytes; expected %ld, got %d\n", sizeof(pi), nb);
        Exit(1);
    }
...
void process_psem(int pid, int32_t fd) {
    int nb;
 ...
 */
    ...
    if (nb <= 0) {
        ...
        (void)fprintf(stderr, "%s: PID %d, FD %d; proc_pidfdinfo(PROC_PIDFDPSEMINFO);\n",
                      ProgramName, pid, fd);
        (void)fprintf(stderr, "      too few bytes; expected %ld, got %d\n", sizeof(ps), nb);
        Exit(1);
    }
...
static void process_pshm_common(struct pshm_fdinfo *ps) {
...
void process_pshm(int pid, int32_t fd) {
    int nb;
 ...
 */
    ...
    if (nb <= 0) {
        ...
        (void)fprintf(stderr, "%s: PID %d, FD %d; proc_pidfdinfo(PROC_PIDFDPSHMINFO);\n",
                      ProgramName, pid, fd);
        (void)fprintf(stderr, "      too few bytes; expected %ld, got %d\n", sizeof(ps), nb);
        Exit(1);
    }
...
static void process_vnode_common(struct vnode_fdinfowithpath *vi) {
    /*
...
void process_vnode(int pid, int32_t fd) {
    int nb;
    ...
    if (nb <= 0) {
        ...
        (void)fprintf(stderr, "%s: PID %d, FD %d: proc_pidfdinfo(PROC_PIDFDVNODEPATHINFO);\n",
                      ProgramName, pid, fd);
        (void)fprintf(stderr, "      too few bytes; expected %ld, got %d\n", sizeof(vi), nb);
        Exit(1);
    }
