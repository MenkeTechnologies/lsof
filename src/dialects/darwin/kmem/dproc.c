 ...
 */
    for (p = Proc, px = 0; px < Np; p++, px++) {
        ...
        if (is_cmd_excl(cmd, &pss, &sf))
            continue;
        alloc_lproc(p->p_pid, pgid, ppid, (UID_ARG)uid, cmd, (int)pss, (int)sf);
...
#if DARWINV >= 700
static char *getcmdnm(pid_t pid) {
    static int am;
...
static pid_t get_parent_pid(KA_T kpa) {
    struct phash *ph;
...
static void process_map(pid_t pid) {
 ...
 */
    if (task_for_pid((mach_port_name_t)mach_task_self(), pid, (mach_port_name_t *)&task) !=
        KERN_SUCCESS) {
        return;
    }
    /*
 ...
 */
    for (address = 0;; address += size) {
        ...
        if (vm_region(task, &address, &size, VM_REGION_EXTENDED_INFO, (vm_region_info_t)&e_info,
        ...
        if (vm_region(task, &address, &size, VM_REGION_TOP_INFO, (vm_region_info_t)&t_info, &count,
 ...
 */
    for (i = Np = 0, kp = Akp, p = Proc, pe += pe; kp && i < pe; i++, kp = kpn) {
        if (kread(kp, (char *)p, sizeof(struct proc)))
            break;
        kpn = (KA_T)(((KA_T)p->p_list.le_next == Akp) ? NULL : p->p_list.le_next);
        ...
        if (Np >= Npa) {
            ...
            if (!(Pa = (KA_T *)realloc((MALLOC_P *)Pa, msz))) {
                (void)fprintf(stderr, "%s: no additional proc addr space\n", ProgramName);
                Exit(1);
            }
            Npa += PINCRSZ;
        }
