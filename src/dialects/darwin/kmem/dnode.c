...
static int lkup_dev_tty(dev_t *dr, dev_t *rdr, INODETYPE *ir) {
    int i;
 ...
 */
    ...
    case VT_DEVFS:
        if (!v->v_data || kread((KA_T)v->v_data, (char *)&db, sizeof(db))) {
...
#if defined(HASFDESCFS)
    case VT_FDESC:
        if (!v->v_data || kread((KA_T)v->v_data, (char *)&fb, sizeof(fb))) {
    ...
    case VT_HFS:
        ...
        if (NodeType != N_AFPFS) {
            ...
            if (!v->v_data || kread((KA_T)v->v_data, (char *)&hb, sizeof(hb))) {
...
#if DARWINV < 600
            if (!h->h_meta || kread((KA_T)h->h_meta, (char *)&hmb, sizeof(hmb))) {
        ...
        if (h->c_rsrc_vp == v)
            ...
            hf = h->c_datafork;
        if (!hf || kread((KA_T)hf, (char *)&hfb, sizeof(hfb))) {
...
#if DARWINV < 600
                    CurrentLocalFile->sz =
                        (type == VDIR) ? (SZOFFTYPE)hm->h_size : (SZOFFTYPE)h->fcbEOF;
#else  /* DARWINV>=600 */
                    if (type == VDIR)
 ...
 */
    ...
    if ((vn = getvpath(va, v))) {
        ...
        if (!statsafely(vn, &sb)) {
             ...
             */
            (void)snpf(NameChars, NameCharsLength, "stat(%s): %s", vn, strerror(errno));
            (void)free((FREE_P *)vn);
        }
        /*
         * Record an NFS file.
...
static int readvname(KA_T addr, char *buf, int buflen) {
    int n, rl;
    /*
