 ...
 */
    if (!is_readable(path, 0)) {
        if (!OptWarnings)
            (void)fprintf(stderr, "%s: WARNING: can't access AFS name list file: %s\n", ProgramName,
                          path);
        return;
    }
    /*
 ...
 */
    if (nlist(path, nl) < 0)
        return;
    for (i = 0; NlistTable[i].n_name && NlistTable[i].n_name[0]; i++) {
    ...
    /*
 * If any AFS symbol that doesn't have a value in NlistTable[] has one from
 * the AFS modload file, copy its modload value to NlistTable[].
 */
    if ((i = get_Nl_value("arFid", Drive_Nl, &v)) >= 0 && !NlistTable[i].n_value && nl[i].n_value)
        NlistTable[i].n_value = nl[i].n_value;
    if ((i = get_Nl_value("avops", Drive_Nl, &v)) >= 0 && !NlistTable[i].n_value && nl[i].n_value)
        NlistTable[i].n_value = nl[i].n_value;
    if ((i = get_Nl_value("avol", Drive_Nl, &v)) >= 0 && !NlistTable[i].n_value && nl[i].n_value)
        NlistTable[i].n_value = nl[i].n_value;
}
...
static struct volume *getvolume(struct VenusFid *f, int *vols) {
    ...
    if (!ka) {
        if (get_Nl_value("avol", Drive_Nl, &ka) < 0 || !ka) {
            if (!w && !OptWarnings) {
                (void)fprintf(stderr, "%s: WARNING: no kernel address for afs_volumes\n",
                              ProgramName);
 ...
 */
    for (mp = readmnt(), n = 0; mp; mp = mp->next) {
        if (AFSdevStat && mp->dev == AFSdev && mp->dir && strcmp(mp->dir, "/afs") == 0 &&
...
static int is_rootFid(struct vcache *vc, int *rfid) {
    ...
    case 0:
        ...
        } else if (kread(arFid, (char *)&r, sizeof(r))) {
...
int readafsnode(KA_T va, struct vnode *v, struct afsnode *an) {
    char *cp, tbuf[32];
    KA_T ka;
    int len, rfid, vols;
    struct vcache *vc;
    ...
    if (kread(ka, cp, len)) {
        (void)snpf(NameChars, NameCharsLength, "vnode at %s: can't read vcache remainder from %s",
                   print_kptr(va, tbuf, sizeof(tbuf)), print_kptr(ka, (char *)NULL, 0));
        enter_nm(NameChars);
        return (1);
    }
    vc = (struct vcache *)v;
 ...
 */
    if (vc->mvstat == 2) {
        if ((vp = getvolume(&vc->fid, &vols))) {
            an->inode =
                (INODETYPE)((vp->mtpoint.Fid.Vnode + (vp->mtpoint.Fid.Volume << 16)) & 0x7fffffff);
        ...
        an->inode = (INODETYPE)((vc->fid.Fid.Vnode + (vc->fid.Fid.Volume << 16)) & 0x7fffffff);
        an->ino_st = 1;
    }
    return (0);
}
