...
_PROTOTYPE(extern int CTF_getmem,
           (ctf_file_t * f, const char *mod, const char *ty, CTF_member_t *mem));
_PROTOTYPE(extern void CTF_init, (int *i, char *t, CTF_request_t *r));
_PROTOTYPE(extern int CTF_memCB, (const char *name, ctf_id_t id, ulong_t offset, void *arg));
...
_PROTOTYPE(extern int read_vxnode,
           (KA_T va, struct vnode *v, struct l_vfs *vfs, int fx, struct l_ino *li, KA_T *vnops));
...
#if solaris >= 110000
_PROTOTYPE(extern int process_VSOCK, (KA_T va, struct vnode *v, struct sonode *so));
#endif /* solaris>=11000 */
