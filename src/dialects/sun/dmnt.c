...
static char *getmntdev(char *o, int l, struct stat *s, char *f) {
     ...
     */
    s->st_dev =
        (((s->st_dev >> L_BITSMINOR32) & L_MAXMAJ32) << L_BITSMINOR) | (s->st_dev & L_MAXMIN32);
 ...
 */
    ...
    for (mp = &me; getmntent(mfp, mp) == 0;) {
         ...
         */
        if (!AFSdevStat && mtp->dir && strcmp(mtp->dir, "/afs") == 0 && mtp->fsname &&
            strcmp(mtp->fsname, "AFS") == 0) {
            AFSdev = mtp->dev;
            AFSdevStat = 1;
        }
    ...
    if (!OptWarnings && zwarn) {
        (void)fprintf(stderr, "%s: WARNING: can't stat() %d zone file system%s", ProgramName, zwarn,
                      (zwarn == 1) ? "" : "s");
...
struct l_vfs *readvfs(KA_T ka, struct vfs *la, struct vnode *lv) {
    struct vfs *v, tv;
    struct l_vfs *vp;
