...
static int Devx = 0; /* current DeviceTable[] index */
...
/*
 * make_devtp() - make DeviceTable[] entry
 */

static void make_devtp(struct stat *s, char *p) {

    /*
 * Make room for another DeviceTable[] entry.
 */
    if (Devx >= NumDevices) {
        NumDevices += DEVINCR;
        if (!DeviceTable)
            DeviceTable = (struct l_dev *)malloc((MALLOC_S)(sizeof(struct l_dev) * NumDevices));
        else
    ...
    /*
 * Store the device number, inode number, and name in the DeviceTable[] entry.
 */
    DeviceTable[Devx].inode = (INODETYPE)s->st_ino;
    if (!(DeviceTable[Devx].name = mkstrcpy(p, (MALLOC_S *)NULL))) {
        (void)fprintf(stderr, "%s: no space for /dev/", ProgramName);
        safestrprt(p, stderr, 1);
        Exit(1);
    }
    DeviceTable[Devx].rdev = s->st_rdev;
    DeviceTable[Devx].v = 0;
    Devx++;
}
 ...
 */
    for (dp = ReadDir(dfp); dp; dp = ReadDir(dfp)) {
        ...
        if (!(fp = mkstrcat(path, pl, dp->d_name, -1, (char *)NULL, -1, &fpl))) {
         ...
         */
        if ((sb.st_mode & S_IFMT) == S_IFDIR || (sb.st_mode & S_IFMT) != S_IFCHR)
            continue;
        /*
         * Make DeviceTable[] entry.
         */
        make_devtp(&sb, fp);
        /*
        ...
        if (strncmp(&fp[pl], "clone", 5) == 0) {
            ...
            if (!(c = (struct clone *)malloc(sizeof(struct clone)))) {
                (void)fprintf(stderr, "%s: no space for network clone device: ", ProgramName);
            ...
            if (!(c->cd.name = mkstrcpy(fp, (MALLOC_S *)NULL))) {
                (void)fprintf(stderr, "%s: no space for clone name: ", ProgramName);
                safestrprt(fp, stderr, 1);
                Exit(1);
            }
            /*
        ...
        if (GET_MIN_DEV(sb.st_rdev) == 0) {
            ...
            if (!(p = (struct pseudo *)malloc(sizeof(struct pseudo)))) {
                (void)fprintf(stderr, "%s: no space for pseudo device: ", ProgramName);
                safestrprt(fp, stderr, 1);
                Exit(1);
            }
            /*
...
void readdev(int skip) {
    ...
    if (!(ppath = mkstrcat(DVCH_DEVPATH, -1, "/", 1, "pseudo", -1, (MALLOC_S *)NULL))) {
 ...
 */
    while (--DirStackIndex >= 0) {
        ...
        if (!(path = mkstrcat(DirStack[DirStackIndex], -1, "/", 1, (char *)NULL, -1, &pl))) {
            (void)fprintf(stderr, "%s: no space for: ", ProgramName);
            safestrprt(DirStack[DirStackIndex], stderr, 1);
            Exit(1);
        }
 ...
 */
    ...
    if (BlockNumDevices) {
        if (BlockNumDevices > j) {
            BlockNumDevices = j;
            BlockDeviceTable = (struct l_dev *)realloc(
                (MALLOC_P *)BlockDeviceTable, (MALLOC_S)(sizeof(struct l_dev) * BlockNumDevices));
        }
        if (!(BlockSortedDevices =
                  (struct l_dev **)malloc((MALLOC_S)(sizeof(struct l_dev *) * BlockNumDevices)))) {
            (void)fprintf(stderr, "%s: no space for block device sort pointers\n", ProgramName);
            Exit(1);
        }
        ...
        BlockNumDevices = rmdupdev(&BlockSortedDevices, BlockNumDevices, 0);
    } else {
        if (!OptWarnings)
    ...
    if (NumDevices) {
        ...
        if (!(SortedDevices =
            ...
            (void)fprintf(stderr, "%s: no space for character device sort pointers\n", ProgramName);
            Exit(1);
        }
        for (i = 0; i < NumDevices; i++) {
            SortedDevices[i] = &DeviceTable[i];
        }
        (void)qsort((QSORT_P *)SortedDevices, (size_t)NumDevices, (size_t)sizeof(struct l_dev *),
                    compdev);
        NumDevices = rmdupdev(&SortedDevices, NumDevices, 1);
    } else {
...
int rw_clone_sect(int m) {
     ...
     */
        for (i = 0; i < n; i++) {
            if (!fgets(buf, sizeof(buf), DevCacheStream)) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: bad clone line in %s: ", ProgramName,
                                  DevCachePath[DevCachePathIndex]);
                    safestrprt(buf, stderr, 1);
                }
                return (1);
         ...
         */
            if (!(c = (struct clone *)calloc(1, sizeof(struct clone)))) {
                (void)fprintf(stderr, "%s: no space for cached clone: ", ProgramName);
                safestrprt(buf, stderr, 1);
                Exit(1);
            }
            /*
         ...
         */
            if (buf[0] != ' ' || !(cp = x2dev(&buf[1], &c->cd.rdev)) || *cp++ != ' ') {
                if (!OptWarnings) {
         ...
         */
            for (c->n = 0; *cp != ' '; cp++) {
                if (*cp < '0' || *cp > '9') {
                    if (!OptWarnings) {
                        (void)fprintf(stderr, "%s: bad cached clone network flag: ", ProgramName);
                        safestrprt(buf, stderr, 1);
                    }
                    return (1);
                }
         ...
         */
            for (c->cd.inode = (INODETYPE)0, ++cp; *cp != ' '; cp++) {
                if (*cp < '0' || *cp > '9') {
                    if (!OptWarnings) {
                        (void)fprintf(stderr, "%s: bad cached clone inode number: ", ProgramName);
                        safestrprt(buf, stderr, 1);
                    }
                    return (1);
                }
                c->cd.inode = (INODETYPE)((c->cd.inode * 10) + (int)(*cp - '0'));
            }
            /*
         ...
         */
            if ((len = strlen(++cp)) < 2 || *(cp + len - 1) != '\n') {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: bad cached clone path: ", ProgramName);
                    safestrprt(buf, stderr, 1);
                }
                return (1);
            }
            *(cp + len - 1) = '\0';
            if (!(c->cd.name = mkstrcpy(cp, (MALLOC_S *)NULL))) {
                (void)fprintf(stderr, "%s: no space for cached clone path: ", ProgramName);
                safestrprt(buf, stderr, 1);
                Exit(1);
            }
            c->cd.v = 0;
...
int rw_pseudo_sect(int m) {
     ...
     */
        for (i = 0; i < n; i++) {
            if (!fgets(buf, sizeof(buf), DevCacheStream)) {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: bad pseudo line in %s: ", ProgramName,
                                  DevCachePath[DevCachePathIndex]);
                    safestrprt(buf, stderr, 1);
                }
                return (1);
         ...
         */
            if (!(p = (struct pseudo *)calloc(1, sizeof(struct pseudo)))) {
                (void)fprintf(stderr, "%s: no space for cached pseudo: ", ProgramName);
                safestrprt(buf, stderr, 1);
                Exit(1);
            }
            /*
         ...
         * not use the new format lines.
         */
            if (buf[0] != ' ' || !(cp = x2dev(&buf[1], &p->pd.rdev)) || *cp++ != ' ') {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: bad cached pseudo device: ", ProgramName);
                    safestrprt(buf, stderr, 1);
                }
                return (1);
            }
         ...
         */
            for (p->pd.inode = (INODETYPE)0; *cp != ' '; cp++) {
                if (*cp < '0' || *cp > '9') {
                    if (!OptWarnings) {
                        (void)fprintf(stderr, "%s: bad cached pseudo inode number: ", ProgramName);
                        safestrprt(buf, stderr, 1);
                    }
                    return (1);
                }
                p->pd.inode = (INODETYPE)((p->pd.inode * 10) + (int)(*cp - '0'));
            }
            /*
         ...
         */
            if ((len = strlen(++cp)) < 2 || *(cp + len - 1) != '\n') {
                if (!OptWarnings) {
                    (void)fprintf(stderr, "%s: bad cached pseudo path: ", ProgramName);
                    safestrprt(buf, stderr, 1);
                }
                return (1);
            }
            if (!(p->pd.name = (char *)malloc(len))) {
                (void)fprintf(stderr, "%s: no space for cached pseudo path: ", ProgramName);
                safestrprt(buf, stderr, 1);
                Exit(1);
            }
            *(cp + len - 1) = '\0';
...
static int rmdupdev(struct l_dev ***dp, int n, int ty) {
    ...
    for (i = j = 0, d = *dp; i < n;) {
        for (k = i + 1; k < n; k++) {
             ...
             */
            for (p = Pseudo, pp = (struct pseudo *)NULL; p; pp = p, p = p->next) {
    ...
    if (n == j)
        return (n);
    if (!(*dp =
              (struct l_dev **)realloc((MALLOC_P *)*dp, (MALLOC_S)(j * sizeof(struct l_dev *))))) {
