...
static int bench_compare_devices(const void *first, const void *second) {
    struct bench_l_dev **dev_ptr1 = (struct bench_l_dev **)first;
    struct bench_l_dev **dev_ptr2 = (struct bench_l_dev **)second;
    if ((*dev_ptr1)->rdev < (*dev_ptr2)->rdev)
        return -1;
    if ((*dev_ptr1)->rdev > (*dev_ptr2)->rdev)
        return 1;
    if ((*dev_ptr1)->inode < (*dev_ptr2)->inode)
        return -1;
    if ((*dev_ptr1)->inode > (*dev_ptr2)->inode)
        return 1;
...
BENCH(device_bsearch_100, 5000000) {
    ...
    for (int i = 0; i < bf_iters; i++) {
        ...
        while (lo <= hi) {
            int mid = (lo + hi) / 2;
            if (devices[mid] < target)
                lo = mid + 1;
            else if (devices[mid] > target)
                hi = mid - 1;
            else {
                found = 1;
                break;
